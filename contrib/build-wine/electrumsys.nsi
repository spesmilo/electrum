...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/syscoin/electrumsys"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrumsys-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrumsys\electrumsys\gui\icons\electrumsys.ico"
...
Section
  ...
  File /r "dist\electrumsys\*.*"
  File "c:\electrumsys\electrumsys\gui\icons\electrumsys.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe" 0


  ;Links syscoin: URI's to ElectrumSys
  WriteRegStr HKCU "Software\Classes\syscoin" "" "URL:syscoin Protocol"
  WriteRegStr HKCU "Software\Classes\syscoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\syscoin" "DefaultIcon" "$\"$INSTDIR\electrumsys.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\syscoin\shell\open\command" "" "$\"$INSTDIR\electrumsys-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrumsys.ico"
