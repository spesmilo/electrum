...
clone_or_update_repo() {
    ...
    if [ -d "$repo_dir" ]; then
        ...
        git -C "$repo_dir" reset --hard "$commit_hash" >/dev/null 2>&1 || fail "Failed to reset to commit $commit_hash"
    else
        ...
        git -C "$repo_dir" checkout "$commit_hash" >/dev/null 2>&1 || fail "Failed to checkout commit $commit_hash"
    fi
}
