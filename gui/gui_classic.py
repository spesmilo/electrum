...
class ElectrumWindow(QMainWindow):
    ...
    def init_menubar(self):
        ...
        csv_transaction_menu = electrum_menu.addMenu(_("&Load CSV transaction"))
    ...
    def do_process_from_csvReader(self, csvReader):
        ...
        try:
            ...
            QMessageBox.critical(None,"Unable to read file or no transaction found", _("Electrum was unable to open your transaction file") + "\n" + str(reason))
            return
        # Generate transaction (temporarily axing wallet seed to blcok signing)
        saved_seed = self.wallet.seed
        self.wallet.seed = None
        try:
            tx = self.wallet.mktx(outputs, None, None, account=self.current_account)
        except BaseException, e:
            # Restore seed
            self.wallet.seed = saved_seed
            self.show_message(str(e))
            return
        self.wallet.seed = saved_seed
        tx_dict = tx.as_dict()
