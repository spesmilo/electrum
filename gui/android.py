...
def get_history_values(n):
    ...
    for i in range(length):
        ...
        values.append(
            conf_str, '  ' + time_str, '  ' + format_satoshis(value, True), '  ' + label)
    ...
    while out is None:
        ...
        if event["name"] == "click":
            ...
            if out == 'contacts':
                ...
                if receive_addr:
                    amount = modal_input(
                        'Amount', 'Amount you want receive. ', '', "numberDecimal")
                    if amount:
                        receive_addr = 'bitcoin:%s?amount=%s' % (
                            receive_addr, amount)
    ...
    while out is None:
        ...
        if event["name"] == "click":
            ...
            if id == "buttonPay":
                ...
                droid.fullQuery()
                recipient = droid.fullQueryDetail(
                    "recipient").result.get('text')
...
def settings_loop():
    ...
    def set_listview():
        ...
        droid.fullSetList("myListView", ['Server: ' + host, 'Protocol: ' + protocol,
                          'Port: ' + port, 'Transaction fee: ' + fee, 'Password: ' + is_encrypted, 'Seed'])
    ...
    while out is None:
        ...
        if name == "itemclick":
            ...
            if pos == "0":  # server
                ...
                a_port = modal_input(
                    'Port number', 'If you use a public server, this field is set automatically when you set the protocol', port, "number")
                if a_port != port:
                    port = a_port
                    network_changed = True

            elif pos == "3":  # fee
                fee = modal_input('Transaction fee', 'The fee will be this amount multiplied by the number of inputs in your transaction. ', str(
                    Decimal(wallet.fee) / 100000000), "numberDecimal")
                if fee:
                    try:
...
class ElectrumGui:
    ...
    def restore_or_create(self):
        droid.dialogCreateAlert(
            "Wallet not found", "Do you want to create a new wallet, or restore an existing one?")
