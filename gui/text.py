...
class ElectrumGui:
    ...
    def __init__(self, config, network):
        ...
        self.tab_names = [
            _("History"), _("Send"), _("Receive"), _("Contacts"), _("Wall")]
    ...
    def print_history(self):
        ...
        self.print_list(self.history[::-1], format_str %
    ...
    def update_history(self):
        ...
        for item in self.wallet.get_tx_history():
            ...
            self.history.append(format_str % (time_str, label, format_satoshis(
    ...
    def print_balance(self):
        ...
        for i in range(self.num_tabs):
            self.stdscr.addstr(
                0, 2 + 2 * i + len(''.join(self.tab_names[0:i])), ' ' + self.tab_names[i] + ' ', curses.A_BOLD if self.tab == i else 0)
        ...
        while True:
            ...
            for i in range(num):
                ...
                if item.has_key('value'):
                    ...
                    w.addstr(2 + interval * i, 15, value, curses.A_REVERSE if self.popup_pos %
                             numpos == i else curses.color_pair(1))
                else:
                    w.addstr(
                        2 + interval * i, 2, label, curses.A_REVERSE if self.popup_pos % numpos == i else 0)

            if buttons:
                w.addstr(5 + interval * i, 10, "[  ok  ]",     curses.A_REVERSE if self.popup_pos %
                         numpos == (numpos - 2) else curses.color_pair(2))
                w.addstr(5 + interval * i, 25, "[cancel]", curses.A_REVERSE if self.popup_pos %
