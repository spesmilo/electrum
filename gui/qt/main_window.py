...
default_column_widths = {"history": [40, 140, 350, 140], "contacts": [
    350, 330], "receive": [370, 200, 130]}
...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network):
        ...
        self.column_widths = self.config.get(
            "column_widths_2", default_column_widths)
        ...
        QShortcut(QKeySequence("Ctrl+PgUp"), self, lambda:
                  tabs.setCurrentIndex((tabs.currentIndex() - 1) % tabs.count()))
        QShortcut(QKeySequence("Ctrl+PgDown"), self, lambda:
                  tabs.setCurrentIndex((tabs.currentIndex() + 1) % tabs.count()))
        ...
        self.connect(self, QtCore.SIGNAL('banner_signal'),
                     lambda: self.console.showMessage(self.network.banner))
        self.connect(self, QtCore.SIGNAL('transaction_signal'),
                     lambda: self.notify_transactions())
        ...
        if self.network:
            ...
            self.network.register_callback(
                'banner', lambda: self.emit(QtCore.SIGNAL('banner_signal')))
            self.network.register_callback(
                'disconnected', lambda: self.emit(QtCore.SIGNAL('update_status')))
            self.network.register_callback(
                'disconnecting', lambda: self.emit(QtCore.SIGNAL('update_status')))
            self.network.register_callback(
                'new_transaction', lambda: self.emit(QtCore.SIGNAL('transaction_signal')))
    ...
    def update_account_selector(self):
        ...
        if len(accounts) > 1:
            self.account_selector.addItems(
                [_("All accounts")] + accounts.values())
    ...
    def open_wallet(self):
        ...
        filename = unicode(
            QFileDialog.getOpenFileName(self, "Select your wallet file", wallet_folder))
        if not filename:
            return
    ...
    def backup_wallet(self):
        ...
        if new_path != path:
            try:
                ...
                QMessageBox.information(None, "Wallet backup created", _(
                    "A copy of your wallet file was created in") + " '%s'" % str(new_path))
            except (IOError, os.error), reason:
                QMessageBox.critical(None, "Unable to create backup", _(
                    "Electrum was unable to copy your wallet file to the specified location.") + "\n" + str(reason))
    ...
    def new_wallet(self):
        ...
        filename = unicode(
            QFileDialog.getSaveFileName(self, _('Enter a new file name'), wallet_folder))
        if not filename:
            return
    ...
    def init_menubar(self):
        ...
        # Settings / Preferences are all reserved keywords in OSX using this as
        # work around
        preferences_name = _(
            "Electrum preferences") if sys.platform == 'darwin' else _("Preferences")
        ...
        csv_transaction_file = csv_transaction_menu.addAction(
            _("&From CSV file"))
        csv_transaction_file.triggered.connect(self.do_process_from_csv_file)

        csv_transaction_text = csv_transaction_menu.addAction(
            _("&From CSV text"))
        ...
        raw_transaction_text = raw_transaction_menu.addAction(
        ...
        web_open.triggered.connect(
            lambda: webbrowser.open("http://electrum.org"))

        help_menu.addSeparator()
        doc_open = help_menu.addAction(_("&Documentation"))
        doc_open.setShortcut(QKeySequence.HelpContents)
        doc_open.triggered.connect(
            lambda: webbrowser.open("http://electrum.org/documentation.html"))
    ...
    def notify_transactions(self):
        ...
        if len(self.network.interface.pending_transactions_for_notifications) > 0:
            ...
            if(tx_amount >= 3):
                ...
                for tx in self.network.interface.pending_transactions_for_notifications:
                    if tx:
                        self.network.interface.pending_transactions_for_notifications.remove(
                            tx)
                        is_relevant, is_mine, v, fee = self.wallet.get_tx_value(
                            tx)
                        if(v > 0):
                            self.notify(_("New transaction received. %(amount)s %(unit)s") % {
                                        'amount': self.format_amount(v), 'unit': self.base_unit()})

    def notify(self, message):
        self.tray.showMessage(
            "Electrum", message, QSystemTrayIcon.Information, 20000)

    # custom wrappers for getOpenFileName and getSaveFileName, that remember
    ...
    def update_status(self):
        if self.network is None or not self.network.is_running():
            ...
            if not self.wallet.up_to_date:
                ...
                text = _("Server is lagging (%d blocks)" %
                         self.network.server_lag)
                icon = QIcon(":icons/status_lagging.png")
            else:
                c, u = self.wallet.get_account_balance(self.current_account)
                text = _("Balance") + \
                    ": %s " % (self.format_amount(c)) + self.base_unit()
                if u:
                    text += " [%s unconfirmed]" % (self.format_amount(u,
                                                   True).strip())
    ...
    def create_history_tab(self):
        ...
        self.connect(
            l, SIGNAL('itemDoubleClicked(QTreeWidgetItem*, int)'), self.tx_label_clicked)
        self.connect(
            l, SIGNAL('itemChanged(QTreeWidgetItem*, int)'), self.tx_label_changed)
    ...
    def create_history_menu(self, position):
        ...
        menu = QMenu()
        menu.addAction(
            _("Copy ID to Clipboard"), lambda: self.app.clipboard().setText(tx_hash))
        menu.addAction(_("Details"), lambda: self.show_transaction(
            self.wallet.transactions.get(tx_hash)))
        menu.addAction(
            _("Edit description"), lambda: self.tx_label_clicked(item, 2))
    ...
    def create_send_tab(self):
        ...
        def entry_changed(is_fee):
            ...
            if self.amount_e.is_shortcut:
                ...
                inputs, total, fee = self.wallet.choose_tx_inputs(
                    sendable, 0, self.get_payment_sources())
            ...
            if amount is None:
                return
            inputs, total, fee = self.wallet.choose_tx_inputs(
                amount, fee, self.get_payment_sources())
            if not is_fee:
                self.fee_e.setText(self.format_amount(fee))
            if inputs:
                palette = QPalette()
                palette.setColor(
                    self.amount_e.foregroundRole(), QColor('black'))
                text = ""
            else:
                palette = QPalette()
                ...
                if c + u:
                    text += ' (' + self.format_amount(c + u).strip() + \
    ...
    def do_send(self):
        ...
        if not is_valid(to_address):
            QMessageBox.warning(
                self, _('Error'), _('Invalid Bitcoin Address') + ':\n' + to_address, _('OK'))
            return
    ...
    def send_tx(self, to_address, amount, fee, label, password):
        ...
        if tx.requires_fee(self.wallet.verifier) and fee < MIN_RELAY_TX_FEE:
            QMessageBox.warning(self, _('Error'), _(
                "This transaction requires a higher fee, or it will not be propagated by the network."), _('OK'))
            return
        ...
        if tx.is_complete:
            ...
            waiting_dialog(
                lambda: False if self.wallet.tx_event.isSet() else _("Please wait..."))
    ...
    def set_url(self, url):
        address, amount, label, message, signature, identity, url = util.parse_url(
            url)

        try:
            if amount and self.base_unit() == 'mBTC':
        ...
        if identity:
            ...
            self.payto_sig.setText(
                '      ' + _('The bitcoin URI was signed by') + ' ' + identity)
        else:
    ...
    def create_receive_tab(self):
        l, w, hbox = self.create_list_tab(
            [_('Address'), _('Label'), _('Balance'), _('Tx')])
        l.setContextMenuPolicy(Qt.CustomContextMenu)
        l.customContextMenuRequested.connect(self.create_receive_menu)
        l.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.connect(l, SIGNAL('itemDoubleClicked(QTreeWidgetItem*, int)'),
                     lambda a, b: self.address_label_clicked(a, b, l, 0, 1))
        self.connect(l, SIGNAL('itemChanged(QTreeWidgetItem*, int)'),
                     lambda a, b: self.address_label_changed(a, b, l, 0, 1))
        self.connect(l, SIGNAL('currentItemChanged(QTreeWidgetItem*, QTreeWidgetItem*)'),
                     lambda a, b: self.current_item_changed(a))
        self.receive_list = l
    ...
    def save_column_widths(self):
        ...
        for i in range(self.receive_list.columnCount() - 1):
            self.column_widths["receive"].append(
                self.receive_list.columnWidth(i))

        self.column_widths["history"] = []
        for i in range(self.history_list.columnCount() - 1):
            self.column_widths["history"].append(
                self.history_list.columnWidth(i))

        self.column_widths["contacts"] = []
        for i in range(self.contacts_list.columnCount() - 1):
            self.column_widths["contacts"].append(
                self.contacts_list.columnWidth(i))
    ...
    def edit_account_label(self, k):
        text, ok = QInputDialog.getText(
            self, _('Rename account'), _('Name') + ':', text=self.wallet.labels.get(k, ''))
        if ok:
            label = unicode(text)
    ...
    def create_account_menu(self, position, k, item):
        ...
        if item.isExpanded():
            menu.addAction(
                _("Minimize"), lambda: self.account_set_expanded(item, k, False))
        else:
            menu.addAction(
                _("Maximize"), lambda: self.account_set_expanded(item, k, True))
    ...
    def create_receive_menu(self, position):
        ...
        if not multi_select:
            menu.addAction(
                _("Copy to clipboard"), lambda: self.app.clipboard().setText(addr))
            menu.addAction(
                _("QR code"), lambda: self.show_qrcode("bitcoin:" + addr, _("Address")))
            menu.addAction(_("Edit label"), lambda: self.edit_label(True))
            if self.wallet.seed:
                menu.addAction(
                    _("Private key"), lambda: self.show_private_key(addr))
                menu.addAction(
                    _("Sign message"), lambda: self.sign_message(addr))
            if addr in self.wallet.imported_keys:
                menu.addAction(
                    _("Remove from wallet"), lambda: self.delete_imported_key(addr))

        if any(addr not in self.wallet.frozen_addresses for addr in addrs):
            menu.addAction(
                _("Freeze"), lambda: self.set_addrs_frozen(addrs, True))
        if any(addr in self.wallet.frozen_addresses for addr in addrs):
            menu.addAction(
                _("Unfreeze"), lambda: self.set_addrs_frozen(addrs, False))

        if any(addr not in self.wallet.frozen_addresses for addr in addrs):
            menu.addAction(
                _("Send From"), lambda: self.send_from_addresses(addrs))
    ...
    def update_receive_tab(self):
        ...
        if self.current_account is None:
            ...
            account_items = [
                (self.current_account, self.wallet.accounts.get(self.current_account))]
        else:
            account_items = []
        ...
        if self.wallet.imported_keys and (self.current_account is None or self.current_account == -1):
            ...
            account_item = QTreeWidgetItem(
                [_('Imported'), '', self.format_amount(c + u), ''])
            l.addTopLevelItem(account_item)
            account_item.setExpanded(True)
    ...
    def update_console(self):
        ...
        console.updateNamespace(
            {'wallet': self.wallet, 'network': self.network, 'gui': self})
        console.updateNamespace({'util': util, 'bitcoin': bitcoin})

        c = commands.Commands(
            self.wallet, self.network, lambda: self.console.set_json(True))
        methods = {}

        def mkfunc(f, method):
            return lambda *args: apply(f, (method, args, self.password_dialog))
        for m in dir(c):
            if m[0] == '_' or m in ['network', 'wallet']:
                continue
    ...
    def create_status_bar(self):
        ...
        self.connect(self.account_selector, SIGNAL(
            "activated(QString)"), self.change_account)
        sb.addPermanentWidget(self.account_selector)

        if (int(qtVersion[0]) >= 4 and int(qtVersion[2]) >= 7):
            sb.addPermanentWidget(StatusBarButton(
                QIcon(":icons/switchgui.png"), _("Switch to Lite Mode"), self.go_lite))
        ...
        sb.addPermanentWidget(StatusBarButton(
            QIcon(":icons/preferences.png"), _("Preferences"), self.settings_dialog))
        self.seed_button = StatusBarButton(
            QIcon(":icons/seed.png"), _("Seed"), self.show_seed_dialog)
        sb.addPermanentWidget(self.seed_button)
        self.status_button = StatusBarButton(
            QIcon(":icons/status_disconnected.png"), _("Network"), self.run_network_dialog)
    ...
    def update_lock_icon(self):
        icon = QIcon(
            ":icons/lock.png") if self.wallet.use_encryption else QIcon(":icons/unlock.png")
    ...
    def new_contact_dialog(self):
        ...
        if not is_valid(address):
            QMessageBox.warning(
                self, _('Error'), _('Invalid Address'), _('OK'))
            return
    ...
    def show_master_public_key_old(self):
        ...
        main_layout.addWidget(
            QLabel(_('Your Master Public Key is:')), 0, 0, 1, 2)
    ...
    def show_seed_dialog(self, password):
        if self.wallet.is_watching_only():
            QMessageBox.information(
                self, _('Message'), _('This is a watching-only wallet'), _('OK'))
            return
        ...
        if self.wallet.seed:
            try:
                ...
                QMessageBox.warning(
                    self, _('Error'), _('Incorrect Password'), _('OK'))
                return
    ...
    def show_qrcode(self, data, title=_("QR code")):
        ...
        def print_qr():
            ...
            QMessageBox.information(
                None, _('Message'), _("QR code saved to file") + " " + filename, _('OK'))

        def copy_to_clipboard():
            bmp.save_qrcode(qrw.qr, filename)
            self.app.clipboard().setImage(QImage(filename))
            QMessageBox.information(
                None, _('Message'), _("QR code saved to clipboard"), _('OK'))
    ...
    def do_sign(self, address, message, signature, password):
        ...
        try:
            sig = self.wallet.sign_message(
                str(address.text()), message, password)
    ...
    def sign_message(self, address):
        ...
        hbox.addWidget(b)
        b.clicked.connect(
            lambda: self.do_sign(sign_address, sign_message, sign_signature))
        b = QPushButton(_("Close"))
        b.clicked.connect(d.accept)
        hbox.addWidget(b)
    ...
    def tx_from_text(self, txt):
        ...
        QMessageBox.critical(None, _("Unable to parse transaction"), _(
    ...
    def read_tx_from_file(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to read file or no transaction found"), _(
                "Electrum was unable to open your transaction file") + "\n" + str(reason))
    ...
    def do_process_from_text(self):
        text = text_dialog(
            self, _('Input raw transaction'), _("Transaction:"), _("Load transaction"))
        if not text:
            return
    ...
    def do_process_from_txid(self):
        ...
        txid, ok = QInputDialog.getText(
            self, _('Lookup transaction'), _('Transaction ID') + ':')
        if ok and txid:
            r = self.network.synchronous_get(
                [('blockchain.transaction.get', [str(txid)])])[0]
            if r:
    ...
    def do_process_from_csvReader(self, csvReader):
        ...
        if errors != []:
            ...
            QMessageBox.critical(None, _("Invalid Addresses"), _(
                "ABORTING! Invalid Addresses found:") + "\n\n" + errtext)
            return
    ...
    def do_process_from_csv_file(self):
        fileName = self.getOpenFileName(
            _("Select your transaction CSV"), "*.csv")
        if not fileName:
            return
        try:
            with open(fileName, "r") as f:
    ...
    def do_export_privkeys(self, password):
        ...
        self.show_message("%s\n%s\n%s" % (_("WARNING: ALL your private keys are secret."),  _("Exposing a single private key can compromise your entire wallet!"), _(
        ...
        try:
            ...
            fileName = self.getSaveFileName(
                select_export, 'electrum-private-keys.csv', "*.csv")
            if fileName:
                with open(fileName, "w+") as csvfile:
                    ...
                    for addr in addresses:
                        pk = "".join(
                            self.wallet.get_private_key(addr, password))
            ...
            export_error_label = _(
                "Electrum was unable to produce a private key-export.")
            QMessageBox.critical(
                None, _("Unable to create csv"), export_error_label + "\n" + str(reason))
    ...
    def do_import_labels(self):
        ...
        try:
            ...
            QMessageBox.information(None, _("Labels imported"), _(
                "Your labels were imported from") + " '%s'" % str(labelsFile))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to import labels"), _(
                "Electrum was unable to import your labels.") + "\n" + str(reason))

    def do_export_labels(self):
        labels = self.wallet.labels
        try:
            fileName = self.getSaveFileName(
                _("Select file to save your labels"), 'electrum_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
                QMessageBox.information(None, _("Labels exported"), _(
                    "Your labels where exported to") + " '%s'" % str(fileName))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to export labels"), _(
                "Electrum was unable to export your labels.") + "\n" + str(reason))
    ...
    def do_import_privkey(self, password):
        ...
        text = text_dialog(
            self, _('Import private keys'), _("Enter private keys") + ':', _("Import"))
        if not text:
            return
        ...
        if addrlist:
            QMessageBox.information(self, _('Information'), _(
                "The following addresses were added") + ':\n' + '\n'.join(addrlist))
        if badkeys:
    ...
    def settings_dialog(self):
        ...
        grid.addWidget(HelpButton(
            _('Select which language is used in the GUI (after restart).') + ' '), 1, 2)
        ...
        grid.addWidget(HelpButton(
            _('Using change addresses makes it more difficult for other people to track your transactions.') + ' '), 4, 2)
        ...
        try:
            ...
            QMessageBox.warning(
                self, _('Error'), _('Invalid value') + ': %s' % fee, _('OK'))
            return
        ...
        try:
            ...
            QMessageBox.warning(
                self, _('Error'), _('Invalid value') + ':%s' % nz, _('OK'))
            return
        ...
        if need_restart:
            QMessageBox.warning(self, _('Success'), _(
                'Please restart Electrum to activate the new GUI settings'), _('OK'))
    ...
    def closeEvent(self, event):
        ...
        g = self.geometry()
        self.config.set_key(
            "winpos-qt", [g.left(), g.top(), g.width(), g.height()], True)
        self.save_column_widths()
        self.config.set_key(
            "console-history", self.console.history[-50:], True)
    ...
    def show_account_details(self, k):
        ...
        acctype = '2 of 2' if len(roots) == 2 else '2 of 3' if len(
            roots) == 3 else 'Single key'
