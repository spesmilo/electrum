...
class NetworkDialog(QDialog):
    ...
    def __init__(self, network, config, parent):
        ...
        if parent:
            ...
            if n:
                status = _("Blockchain") + ": " + "%d " % (network.blockchain.height()) + \
                    _("blocks") + ".\n" + \
        ...
        self.server_protocol.connect(
            self.server_protocol, SIGNAL('currentIndexChanged(int)'), self.change_protocol)
        ...
        label = _('Active Servers') if network.irc_servers else _(
            'Default Servers')
        ...
        if not config.is_modifiable('server'):
            for w in [self.server_host, self.server_port, self.server_protocol, self.servers_list_widget]:
                w.setEnabled(False)
        ...
        check_for_disable()
        self.proxy_mode.connect(
            self.proxy_mode, SIGNAL('currentIndexChanged(int)'), check_for_disable)

        if not self.config.is_modifiable('proxy'):
            for w in [self.proxy_host, self.proxy_port, self.proxy_mode]:
                w.setEnabled(False)

        proxy_config = network.proxy if network.proxy else {
    ...
    def init_servers_list(self):
        ...
        for _host, d in self.servers.items():
            if d.get(self.protocol):
                ...
                self.servers_list_widget.addTopLevelItem(
                    QTreeWidgetItem([_host, pruning_level]))
