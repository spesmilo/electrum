...
class TxDialog(QDialog):
            ...
            if is_mine:
                if fee is not None:
                    self.amount_label.setText(
                        _("Amount sent:") + ' %s' % self.parent.format_amount(v - fee) + ' ' + self.parent.base_unit())
                    self.fee_label.setText(
                        _("Transaction fee") + ': %s' % self.parent.format_amount(fee) + ' ' + self.parent.base_unit())
                else:
                    self.amount_label.setText(
                        _("Amount sent:") + ' %s' % self.parent.format_amount(v) + ' ' + self.parent.base_unit())
                    self.fee_label.setText(
                        _("Transaction fee") + ': ' + _("unknown"))
            else:
                self.amount_label.setText(
                    _("Amount received:") + ' %s' % self.parent.format_amount(v) + ' ' + self.parent.base_unit())
        else:
            self.amount_label.setText(
    ...
    def exec_menu(self, position, l):
        ...
        menu = QMenu()
        menu.addAction(
            _("Copy to clipboard"), lambda: self.parent.app.clipboard().setText(addr))
    ...
    def add_io(self, vbox):
        ...
        lines = map(
            lambda x: x[0] + u'\t\t' + self.parent.format_amount(x[1]), self.tx.outputs)
    ...
    def broadcast(self):
        ...
        if result:
            self.show_message(
                _("Transaction successfully sent") + ': %s' % (result_message))
        else:
            self.show_message(
                _("There was a problem sending your transaction:") + '\n %s' % (result_message))
