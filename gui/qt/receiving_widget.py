...
class ReceivingWidget(QTreeWidget):
    ...
    def update_list(self):
        ...
        for address in addresses:
            ...
            # Or just a tx that's why this ugly code duplication is in, will
            # fix
            if len(history) == 1:
                # This means pruned data. If that's the case the address has to
                # been used at one point
                if history[0] == "*":
                    used = "Yes"
                else:
                    for tx_hash in history:
                        tx = self.owner.actuator.g.wallet.transactions.get(
                            tx_hash)
                        if tx:
                            used = "Yes"
            else:
