...
register_command('getaddressbalance',    1, 1, True,  False, False,
                 'Return the balance of an address', 'getaddressbalance <address>')
register_command('getaddresshistory',    1, 1, True,  False, False,
...
register_command('listunspent',          0, 0, True,  False,
                 False, 'Returns the list of unspent inputs in your wallet.')
register_command('getaddressunspent',    1, 1, True,  False,
                 False, 'Returns the list of unspent inputs in your wallet.')
...
register_command('encrypt',              2, -1, False, False, False,
                 'encrypt a message with pubkey', 'encrypt <pubkey> <message>')
register_command('decrypt',              2, -1, False, False, False,
                 'decrypt a message with privkey', 'decrypt <privkey> <message>')
register_command(
    'daemon',               1, 1, True, False, False, 'start/stop daemon')
register_command('getproof',             1, 1, True, False,
                 False, 'get merkle proof', 'getproof <address>')
register_command('getunspentaddress',    2, 2, True, False, False,
                 'get the address of an unspent', 'getunspentaddress <txid> <pos>')
...
class Commands:
    ...
    def getaddressbalance(self, addr):
        b = self.network.synchronous_get(
            [('blockchain.address.get_balance', [addr])])[0]
        return str(Decimal(b) / 100000000)

    def getproof(self, addr):
        p = self.network.synchronous_get(
            [('blockchain.address.get_proof', [addr])])[0]
        out = []
        for i, s in p:
            out.append(i)
        return out
    ...
    def getrawtransaction(self, tx_hash):
        ...
        r = self.network.synchronous_get(
            [('blockchain.transaction.get', [tx_hash])])[0]
        if r:
