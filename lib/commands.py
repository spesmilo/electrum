...
scatter_syntax = 'scatter_make <scatter_limit> <save_path> <fee>\n Start scattering balances above the <scatter_limit> into smaller amounts, if watching only unsigned transactions dump into <save_path> then scatter_sign, scatter_broadcast must follow'
...
register_command('scatter_make',             1, 1, False, True,  False, 'Create transactions to scatter balances into smaller amounts',scatter_syntax)
...
register_command('isscatteron',          0, 0, False, False, False,  'Returns the status of the scatter process.')
...
class Commands:
    ...
    def isscatteron(self):
	return "Scatter is " + str(self.wallet.scatter_on)

    def scatter_make(self, scatter_limit, saveFolder = '~', fee = 0.0001, minimum_delay = 60) :
	self.wallet.scatter_fee = int(Decimal(str(fee))*100000000)
	self.wallet.scatter_limit = int(Decimal(str(scatter_limit))*100000000)
	self.wallet.scatter_path = os.path.expanduser(saveFolder)
	self.wallet.scatter_delay_minimum = minimum_delay
	return self.wallet.scatter_balances(scatter_limit, saveFolder, fee)
