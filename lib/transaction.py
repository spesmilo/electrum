    ...
    def __init__(self, name, enumList):
        ...
        for x in enumList:
            ...
            if type(x) != types.StringType:
                raise EnumException("enum name is not a string: %s" % x)
            if type(i) != types.IntType:
                raise EnumException("enum value is not an integer: %s" % i)
            if x in uniqueNames:
                raise EnumException("enum name is not unique: %s" % x)
            if i in uniqueValues:
                raise EnumException("enum value is not unique for %s" % x)
...
def get_address_from_input_script(bytes):
    ...
    if match_decoded(decoded, match):
        ...
        #num = len(match) - 2  # 'num' is assigned to but never used
...
class Transaction:
    ...
    def serialize(klass, inputs, outputs, for_sig=None):
        ...
        for i in range(len(inputs)):
            ...
            s += script
            # todo: check to make sure I didn't break this
            s += "ffffffff"  # sequence
        ...
        for i, txin in enumerate(self.inputs):
            ...
            for pubkey in redeem_pubkeys:
                ...
                if pubkey in keypairs.keys():
                    # add signature
                    sec = keypairs[pubkey]
                    #compressed = is_compressed(sec)  # 'compressed' is assigned to but never used
    ...
    def deserialize(self):
        ...
        d = {}
        #start = vds.read_cursor  # never used
    ...
    def parse_input(self, vds):
        ...
        if scriptSig:
            _, signatures, address = get_address_from_input_script(scriptSig)
        else:
            signatures = []
            address = None
