...
class Network(threading.Thread):
    ...
    def __init__(self, config={}):
        ...
        self.daemon = True
        self.config = SimpleConfig(config) if type(
            config) == type({}) else config
        ...
        self.recent_servers = self.config.get(
            'recent_servers', [])  # successful connections
    ...
    def run(self):
        ...
        while self.is_running():
            ...
            if i.is_connected:
                ...
                if i == self.interface:
                    print_error(
                        'sending subscriptions to', self.interface.server)
                    self.send_subscriptions()
                    self.trigger_callback('connected')
            else:
...
if __name__ == "__main__":
    ...
    config = simple_config.SimpleConfig(
        {'verbose': True, 'server': 'ecdsa.org:50002:s'})
