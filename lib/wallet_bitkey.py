...
from bitkey.bitkey_proto import bitkey_pb2 as proto
...
class WalletBitkey:
    ...
    def sign_inputs( self, inputs, outputs, password):
        '''This method adapts parameters for bitkey signtx interface and prepare
            protobuf message'''
        
        '''
           Electrum format:
               FUNCTION INPUT:
                   inputs: [addr, v, p_hash, p_index, p_script, _, _]
                   outputs: [addr, value]
               
               FUNCTION RESULT:
                   [addr, v, p_hash, p_index, p_script, pubkey, sig]
           
           Bitkey format:
               FUNCTION INPUT:
                   inputs: [addr_n, v, p_hash, p_index, p_script]
                   outputs: [addr, addr_n, amount, script_type, script_args]
        ...
        tx.random = os.urandom(256)

        fee = 0
        
        for addr, amount, p_hash, p_index, p_script, _, _ in inputs:
            
            if addr not in self.addresses:
                raise Exception("Only deterministic addresses may be used with bitkey")
            ...
            input = tx.inputs.add()
            input.address_n.append(addr_n)
            input.amount=amount
            input.prev_hash=p_hash.decode('hex')
            input.prev_index = p_index
            input.script_sig=p_script.decode('hex')
