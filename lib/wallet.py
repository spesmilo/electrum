...
class NewWallet:
    ...
    def __init__(self, storage):
        ...
        # wallet.up_to_date is true when the wallet is synchronized (stronger
        # requirement)
    ...
    def create_master_keys(self, account_type, password):
        master_k, master_c, master_K, master_cK = bip32_init(
            self.get_seed(password))
        if account_type == '1of1':
            k0, c0, K0, cK0 = bip32_private_derivation(
                master_k, master_c, "m/", "m/0'/")
            self.master_public_keys["m/0'/"] = (c0, K0, cK0)
            self.master_private_keys["m/0'/"] = pw_encode(k0, password)
        elif account_type == '2of2':
            k1, c1, K1, cK1 = bip32_private_derivation(
                master_k, master_c, "m/", "m/1'/")
            k2, c2, K2, cK2 = bip32_private_derivation(
                master_k, master_c, "m/", "m/2'/")
            ...
            k3, c3, K3, cK3 = bip32_private_derivation(
                master_k, master_c, "m/", "m/3'/")
            k4, c4, K4, cK4 = bip32_private_derivation(
                master_k, master_c, "m/", "m/4'/")
            k5, c5, K5, cK5 = bip32_private_derivation(
                master_k, master_c, "m/", "m/5'/")
    ...
    def next_account(self, account_type='1of1'):
        ...
        if account_type is '1of1':
            ...
            c0, K0, cK0 = bip32_public_derivation(
                master_c0.decode('hex'), master_K0.decode('hex'), "m/0'/", "m/0'/%d" % i)
            account = BIP32_Account({'c': c0, 'K': K0, 'cK': cK0})

        elif account_type == '2of2':
            master_c1, master_K1, _ = self.master_public_keys["m/1'/"]
            c1, K1, cK1 = bip32_public_derivation(
                master_c1.decode('hex'), master_K1.decode('hex'), "m/1'/", "m/1'/%d" % i)
            master_c2, master_K2, _ = self.master_public_keys["m/2'/"]
            c2, K2, cK2 = bip32_public_derivation(
                master_c2.decode('hex'), master_K2.decode('hex'), "m/2'/", "m/2'/%d" % i)
            account = BIP32_Account_2of2(
                {'c': c1, 'K': K1, 'cK': cK1, 'c2': c2, 'K2': K2, 'cK2': cK2})

        elif account_type == '2of3':
            master_c3, master_K3, _ = self.master_public_keys["m/3'/"]
            c3, K3, cK3 = bip32_public_derivation(
                master_c3.decode('hex'), master_K3.decode('hex'), "m/3'/", "m/3'/%d" % i)
            master_c4, master_K4, _ = self.master_public_keys["m/4'/"]
            c4, K4, cK4 = bip32_public_derivation(
                master_c4.decode('hex'), master_K4.decode('hex'), "m/4'/", "m/4'/%d" % i)
            master_c5, master_K5, _ = self.master_public_keys["m/5'/"]
            c5, K5, cK5 = bip32_public_derivation(
                master_c5.decode('hex'), master_K5.decode('hex'), "m/5'/", "m/5'/%d" % i)
            account = BIP32_Account_2of3(
                {'c': c3, 'K': K3, 'cK': cK3, 'c2': c4, 'K2': K4, 'cK2': cK4, 'c3': c5, 'K3': K5, 'cK3': cK5})
    ...
    def get_private_key(self, address, password):
        ...
        if address in self.imported_keys.keys():
            ...
            for root, public_sequence in rs:
                ...
                pk = bip32_private_key(
                    public_sequence, master_k.decode('hex'), master_c.decode('hex'))
                out.append(pk)
            ...
            if keyid:
                ...
                for s in keyid.split('&'):
                    ...
                    if not root:
                        continue
                    sequence = map(
                        lambda x: int(x), sequence.strip('/').split('/'))
        ...
        for txin in tx.inputs:
            for item in unspent_coins:
                if txin['prevout_hash'] == item['prevout_hash'] and txin['prevout_n'] == item['prevout_n']:
                    ...
                    if account != -1:
                        txin['redeemScript'] = self.accounts[
                            account].redeem_script(sequence)
    ...
    def get_address_flags(self, addr):
        flags = "C" if self.is_change(
            addr) else "I" if addr in self.imported_keys.keys() else "-"
    ...
    def choose_tx_inputs(self, amount, fixed_fee, domain=None):
        ...
        for item in coins:
            ...
            fee = self.estimated_fee(
                inputs) if fixed_fee is None else fixed_fee
            if total >= amount + fee:
                break
        else:
            inputs = []
    ...
    def estimated_fee(self, inputs):
        # this assumes non-compressed keys
        estimated_size = len(inputs) * 180 + 80
        ...
        if change_amount > DUST_THRESHOLD:
            if not change_addr:
                ...
                if not self.use_change or account == -1:
                    ...
                    change_addr = self.accounts[account].get_addresses(
                        1)[-self.gap_limit_for_change]
    ...
    def receive_tx_callback(self, tx_hash, tx, tx_height):
        ...
        with self.transaction_lock:
            ...
            if not self.check_new_tx(tx_hash, tx):
                ...
                print_error(
                    "received transaction that is no longer referenced in history", tx_hash)
                return
            self.transactions[tx_hash] = tx
            self.network.interface.pending_transactions_for_notifications.append(
                tx)
    ...
    def receive_history_callback(self, addr, hist):
        ...
        if not self.check_new_history(addr, hist):
            raise Exception(
                "error: received history for %s is not consistent with known transactions" % addr)
    ...
    def get_tx_history(self, account=None):
        ...
        with self.transaction_lock:
            ...
            if balance != c + u:
                result.append(
                    ('', 1000, 0, c + u - balance, None, c + u - balance, None))

            balance = c + u - balance
            for tx_hash, tx in history:
                is_relevant, is_mine, value, fee = self.get_tx_value(
                    tx, account)
                if not is_relevant:
                    continue
                if value is not None:
                ...
                conf, timestamp = self.verifier.get_confirmations(
                    tx_hash) if self.verifier else (None, None)
                result.append(
    ...
    def make_unsigned_transaction(self, outputs, fee=None, change_addr=None, domain=None):
        ...
        outputs = self.add_tx_change(
            inputs, outputs, amount, fee, total, change_addr)
    ...
    def add_input_info(self, inputs):
        for txin in inputs:
            ...
            if redeemScript:
                ...
                txin['redeemPubkey'] = self.accounts[
                    account].get_pubkey(*sequence)
    ...
    def send_tx(self, tx):
        # asynchronous
        self.tx_event.clear()
        self.network.interface.send(
            [('blockchain.transaction.broadcast', [str(tx)])], self.on_broadcast)
        return tx.hash()
    ...
    def set_verifier(self, verifier):
        ...
        vr = self.verifier.transactions.keys(
        ) + self.verifier.verified_tx.keys()
        ...
        for tx_hash, height in old_hist:
            ...
            if not found:
                ...
                for _addr, _v in tx.outputs:
                    ...
                    ext_requests.append(
                        ('blockchain.address.get_history', [_addr]))
    ...
    def restore(self, callback):
        ...
        # wait until we are connected, because the user might have selected
        # another server
        if self.network:
            wait_for_network()
            wait_for_wallet()
        ...
        while self.is_running():
            ...
            for tx_hash, tx_height in missing_tx:
                if (tx_hash, tx_height) not in requested_tx:
                    self.network.send(
                        [('blockchain.transaction.get', [tx_hash, tx_height])], lambda i, r: self.queue.put(r))
            ...
            if method == 'blockchain.address.subscribe':
                ...
                if self.wallet.get_status(self.wallet.get_history(addr)) != result:
                    if requested_histories.get(addr) is None:
                        self.network.send(
                            [('blockchain.address.get_history', [addr])], lambda i, r: self.queue.put(r))
                ...
                if result == ['*']:
                    ...
                    if len(hist) != len(result):
                        raise Exception(
                            "error: server sent history with non-unique txid", result)
                ...
                print_error(
                    "Error: Unknown message:" + method + ", " + repr(params) + ", " + repr(result))
        ...
        for txin in tx.inputs:
            ...
            if keyid:
                ...
                if not m:
                    continue
                mpk = m.group(1)
                if mpk != self.storage.get('master_public_key'):
                    continue
...
class Wallet(object):
    ...
    def __new__(self, storage):
        ...
        if not storage.file_exists:
            seed_version = NEW_SEED_VERSION if config.get(
                'bip32') is True else OLD_SEED_VERSION
        else:
