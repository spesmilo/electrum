...
from account import *  # todo: * imports are lazy and bad. remove them
from bitcoin import *  # todo: * imports are lazy and bad. remove them
from plugins import run_hook
from transaction import Transaction
from util import print_msg, print_error, format_satoshis  # todo: 'format_satoshis' imported but unused
from version import *  # todo: * imports are lazy and bad. remove them
import i18n
import mnemonic
...
class Wallet:
    ...
    def import_key(self, sec, password):
        # check password, but don't save the seed
    ...
    def signrawtransaction(self, tx, input_info, private_keys, password):
        # check that the password is correct, but don't keep the seed
        self.get_seed(password)
    ...
    def get_num_tx(self, address):
        ...
        for tx in self.transactions.values():
            if address in map(lambda x: x[0], tx.outputs):
                n += 1
        return n
    ...
    def choose_tx_inputs(self, amount, fixed_fee, domain=None):
        ...
        for item in coins:
            if item.get('coinbase') and item.get('height') + COINBASE_MATURITY > self.network.blockchain.height:
                continue
            #addr = item.get('address')  # never used
            v = item.get('value')
            total += v
            inputs.append(item)
    ...
    def get_default_label(self, tx_hash):
        ...
        if tx:
            ...
            if is_mine:
                ...
                if o_addr:
                    #dest_label = self.labels.get(o_addr)  # this isn't used
                    try:
                        default_label = self.labels[o_addr]
                    except KeyError:
                        default_label = o_addr
    ...
    def restore(self, callback):
        ...
        def wait_for_wallet():
            ...
            while not self.is_up_to_date():
                msg = "%s\n%s %d\n%s %.1f" % (
                    i18n._("Please wait..."),
                    i18n._("Addresses generated:"),
                    len(self.addresses(True)),
                    i18n._("Kilobytes received:"),
