...
is_seed = lambda x: hmac_sha_512("Seed version", x).encode(
    'hex')[0:2].startswith(SEED_PREFIX)
    ...
    def encrypt_message(self, message, pubkey):
        ...
        if len(pubkey) == 33:  # compressed
            pk = Point(curve_secp256k1, str_to_long(pubkey[1:33]), ECC_YfromX(
                str_to_long(pubkey[1:33]), curve_secp256k1, pubkey[0] == '\x03')[0], _r)
        else:
            pk = Point(curve_secp256k1, str_to_long(
                pubkey[1:33]), str_to_long(pubkey[33:65]), _r)
