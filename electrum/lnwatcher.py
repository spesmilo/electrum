...
class LNWatcher(AddressSynchronizer):
    ...
    def __init__(self, network: 'Network'):
        ...
        # [funding_outpoint_str][prev_txid] -> set of LightningTransaction
        ...
        for funding_outpoint, ctxs in storage.get('sweepstore', {}).items():
            for txid, set_of_txns in ctxs.items():
                for e_tx in set_of_txns:
                    e_tx2 = LightningTransaction.from_json(e_tx)
    ...
    def add_sweep_tx(self, funding_outpoint: str, prev_txid: str, sweeptx):
        encumbered_sweeptx = LightningTransaction.from_json(sweeptx)
        with self.lock:
