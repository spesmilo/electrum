...
class TestPeer(TestCaseForTestnet):
    ...
    def test_payment_recv_mpp_confusion1(self):
        ...
        async def pay():
            ...
            routes_agen = w1.create_routes_for_payment(
                amount_msat=1000,
                final_total_msat=lnaddr1.get_amount_msat(),
                invoice_pubkey=lnaddr1.pubkey.serialize(),
                min_cltv_expiry=lnaddr1.get_min_final_cltv_expiry(),
                r_tags=lnaddr1.get_routing_info('r'),
                invoice_features=lnaddr1.get_features(),
                payment_hash=lnaddr1.paymenthash,
                payment_secret=lnaddr1.payment_secret,
                trampoline_fee_level=1,
                use_two_trampolines=False,
            )
            route = (await routes_agen.__anext__())[0]
            p1.pay(
                route=route,
                chan=alice_channel,
                amount_msat=1000,
    ...
    def test_payment_recv_mpp_confusion2(self):
        """Regression test for https://github.com/spesmilo/electrum/security/advisories/GHSA-8r85-vp7r-hjxf"""
        # This test checks that the following attack does not work:
