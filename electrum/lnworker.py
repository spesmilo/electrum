...
class PaymentInfo:
    ...
    @property
    def db_key(self) -> str:
        return f"{self.payment_hash.hex()}:{int(self.direction)}"
...
class LNWallet(LNWorker):
    ...
    def get_payment_info(self, payment_hash: bytes, *, direction: lnutil.Direction) -> Optional[PaymentInfo]:
        ...
        key = f"{payment_hash.hex()}:{int(direction)}"
        with self.lock:
    ...
    def get_invoice_status(self, invoice: BaseInvoice) -> int:
        ...
        assert isinstance(invoice, (Request, Invoice)), type(invoice_id)
    ...
    def delete_payment_info(self, payment_hash_hex: str, *, direction: lnutil.Direction):
        ...
        with self.lock:
            key = f"{payment_hash_hex}:{int(direction)}"
