...
from .lnutil import (
    get_compressed_pubkey_from_bech32, serialize_htlc_key, deserialize_htlc_key, PaymentFailure,
    generate_keypair,
    LnKeyFamily, LOCAL, REMOTE, MIN_FINAL_CLTV_DELTA_FOR_INVOICE, SENT, RECEIVED, HTLCOwner,
    UpdateAddHtlc, LnFeatures,
    ShortChannelID, HtlcLog, NoPathFound, InvalidGossipMsg, FeeBudgetExceeded,
...
class LNWallet(LNWorker):
    ...
    def delete_payment_bundle(
        ...
        if not payment_key:
            assert payment_hash, "must provide either payment hash or payment key"
    ...
    def update_or_create_mpp_with_received_htlc(
        ...
        *,
        ...
        #   * for forwarded htlcs -> "scid.hex() + ':%d' % htlc_id" [htlc key]
        #   * for final htlcs (we are receiver) -> "payment_hash + payment_secret_from_onion"
        # This also means that sets with htlcs to forward will never contain more than one htlc
        # as the payment key is unique to each htlc.
        #
    ...
    def set_htlc_set_error(
        ...
        if error_code == OnionFailureCode.MPP_TIMEOUT:
            self.set_mpp_resolution(payment_key=payment_key, resolution=RecvMPPResolution.EXPIRED)
        else:
            self.set_mpp_resolution(payment_key=payment_key, resolution=RecvMPPResolution.FAILED)

        return raw_error, error_code, error_data

    def set_mpp_resolution(self, payment_key: str, resolution: RecvMPPResolution) -> ReceivedMPPStatus:
        mpp_status = self.received_mpp_htlcs[payment_key]
        if mpp_status.resolution == resolution:
            return mpp_status
        if resolution == RecvMPPResolution.COMPLETE:
            # don't allow set to be completed if it is already failed,
            # newly received htlcs should already be failed when their set is already failed
            assert not mpp_status.resolution in (RecvMPPResolution.EXPIRED, RecvMPPResolution.FAILED)
        self.logger.info(f'set_mpp_resolution {resolution.name} {len(mpp_status.htlcs)=}: {payment_key=}')
    ...
    async def maybe_forward_htlc_set(
        ...
        assert not payment_key in self.active_forwardings, "cannot forward set twice"
        active_forwarding = self.active_forwardings[payment_key] = []
            ...
            try:
                if trampoline_onion is None:
                    ...
                    if next_htlc:
                        ...
                        active_forwarding.append(next_htlc)
                ...
                assert len(active_forwarding) == 0, f"{active_forwarding=}"
