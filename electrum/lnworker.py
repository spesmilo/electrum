...
class LNWallet(LNWorker):
        ...
        *,
        ...
        try:
            ...
            if funding_tx is not None:
                self.wallet.set_frozen_state_of_tx_inputs(funding_tx, False)
    ...
    async def open_channel_with_peer(
            ...
            password=None,
            freeze_utxos: bool = False) -> Tuple[Channel, PartialTransaction]:
        if self.config.ENABLE_ANCHOR_CHANNELS:
        ...
        funding_tx = self.mktx_for_open_channel(
            ...
            fee_est=None)
        if freeze_utxos:
            self.wallet.set_frozen_state_of_tx_inputs(funding_tx, True)
        try:
