...
from .stored_dict import BaseDB
from .util import MyEncoder

from .stored_dict import Key, key_to_str
...
class JsonCodec:
    ...
    def dumps(value: Any) -> bytes:
        return json.dumps(value, separators=(",", ":"), ensure_ascii=False, cls=MyEncoder).encode("utf-8")
...
class LevelDB(BaseDB):
    """
    Owns the plyvel DB handle. Instantiate once.
    Views (LevelDBDict) share this store.
    """
    ...
    def _full_key(self, path, key: Key) -> bytes:
        return self._prefix_bytes(path + [key])

    def _child_prefix(self, path, key: Key) -> bytes:
        d = self.delimiter.encode("utf-8")
        return self._full_key(path, key) + d

    def _has_children(self, path, key: Key) -> bool:
        db = self.db
        if db is None:
    ...
    def _delete_subtree(self, path, key: Key, wb: Optional[plyvel.WriteBatch] = None) -> None:
        db = self.db
        if db is None:
            raise RuntimeError("DB is closed")
        pfx = self._child_prefix(path, key)
    ...
    def put(self, path, key: Key, value: Any, is_stored_list: bool = False) -> None:
        db = self.db
        if db is None:
            raise RuntimeError("DB is closed")
        with db.write_batch() as wb:
            ...
            if isinstance(value, dict) or is_stored_list:
    ...
    def replace(self, path, key: Key, value: Any) -> None:
    ...
    def list_clear(self, path):
        ...
        #self._delete_subtree(path, key, wb=None)
        self.put(path, key, [], is_stored_list=True)
