...
from electrum.util import profiler, PrintError
...
# General processing of incoming messages
    def inchan_to_inbox(self):
        ...
        try:
            ...
            self.logger.error("Decoding error: {}".format(str(error)))
...
# Processing of normal phases
    ...
    def _parse_from_string_safe(self, msg, sender):
        ...
        try:
            ...
            self.logger.error("Decoding error: {} from sender {}".format(repr(error), sender))
    ...
    def process_verification_and_submission(self):
        ...
        if self.is_inbox_complete(phase):
            ...
            if not res:
                ...
                self.logger.error("Error broadcasting tx: res='{}' status='{}'".format(res, status))
            else:
...
# Miscellaneous functions
    def check_and_blame_insufficient_funds(self):
        ...
        def compute_shuffle_amount():
            ...
            self.log_message("adjusts shuffle amount to {} NAV".format(self.shuffle_amount / 1e8))
    ...
    def check_and_blame_insufficient_funds_phase_5(self, tx):
        ...
        for txin in tx.inputs():
            ...
            if utxo_name in seen_utxos:
                ...
                self.logger.error("***** WARNING: Phase 0 UTXO-is-unique check is to blame here! FIXME!")
                offenders.add(vk)
                continue
            ...
            if is_ok is False: # None indicates blockchain server fault, False indicates actual negative reply
                ...
                if change:
                    ...
                    # should be to blame.
                    self.logger.error("Checking change...", change)
                    try:
                        ...
                        self.logger.error("Server error checking change", change, "unspent list (will proceed to blame):", repr(e))
                        unspent_list = None
                    if unspent_list:
