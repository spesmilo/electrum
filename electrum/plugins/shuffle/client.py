...
from electrum.bitcoin import deserialize_privkey, regenerate_key, EC_KEY, generator_secp256k1, number_to_string
from electrum.address import Address
from electrum.util import InvalidPassword
from electrum.network import Network
from electroncash import networks
from electrum.wallet import dust_threshold
from electrum.simple_config import get_config
    ...
    def wait_for_announcment(self):
        ...
        while self.number_of_players is None:
            ...
            try:
                ...
                self.logger.error("Error parsing packet:", repr(e))
    ...
    def run(self):
        ...
        try:
            try:
                ...
                self.logger.error("Exception in 'run': {}".format(repr(e)))
            ...
            self.logger.error("Unexpected exception in 'run'; traceback follows:\n{}".format(traceback.format_exc()))
            self.logger.send("Error: {}".format(e))
        finally:
    ...
    def join(self, timeout_ignored=None):
        ...
        if self.is_alive():
            ...
            if self.is_alive():
                ...
                self.logger.error("Could not join after 2.0 seconds. Leaving the daemon thread in the background running :(")
                return
            self.logger.error("Joined self")
...
class BackgroundShufflingThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            if self.is_offline_mode():  # aka: '--offline' cmdline arg
                ...
                self.logger.error("Offline mode; thread is alive but will not shuffle any coins.")
        ...
        def _do_check_server(timeout, ssl_verify):
            try:
                ...
                if self.ssl and ssl_verify and not verify_ssl_socket(self.host, self.port, timeout=timeout):
                    self.logger.error("SSL Verification failed")
                    return False
                self.logger.error("Server {}:{} told us that it has shufflePort={} poolSize={} connections={}".format(self.host, self.info_port, self.port, self.poolSize, connections))
                return True
            except BaseException as e:
                self.logger.error("Exception: {}".format(str(e)))
                self.logger.error("Could not query shuffle port for server {}:{} -- defaulting to {}".format(self.host, self.info_port, self.port))
                return False
            finally:
        ...
        if now - self.last_idle_check > self.timeout:
            ...
            for scale, thr in self.threads.items():
                if thr and now - thr.ts > self.timeout:
                    self.logger.error("Thread for scale {} idle timed-out (timeout={}), stopping.".format(scale, self.timeout))
            ...
            with self.wallet.lock:
                ...
                for addr, ts in self._delayed_unreserve_addresses.copy().items():
                    if now - ts > self._delayed_unreserve_timeout:
                        ...
                        ct += 1
            if ct:
                self.logger.error("Freed {} 'delayed unreserve' addresses in {:.02f} msec".format(ct, (time.time()-now)*1e3))
            ...
            while _loopCondition(t0): # _loopCondition modifies timeLeft
                ...
                if isinstance(tup, tuple): # may be None on join()
                    ...
                    if killme:
                        ...
                        #self.logger.error("--> Fwd msg to Qt for: Scale='{}' Sender='{}' Msg='{}'".format(scale, sender, message.strip()))
    ...
    def stop_all_protocol_threads(self, message = "Error: Stop requested"):
        ...
        self._unreserve_addresses()
        if ct:
            self.logger.error("Stopped {} extant threads".format(ct))
        return ct
    ...
    def _unreserve_addresses(self):
        ...
        with self.wallet.lock:
            ...
            if l: self.logger.error("Freed {} reserved addresses".format(l))
            if self.wallet._last_change:
                self.wallet._last_change = None
                self.logger.error("Freed 'last_change'")
    ...
    def stop_protocol_thread(self, thr, scale, sender, message):
        self.logger.error("Stop protocol thread for scale: {}".format(scale))
        retVal = False
        if sender and not thr.already_did_cleanup:
            ...
            with self.wallet.lock:
                ...
                if message.startswith("Error"):
                    if thr.protocol and thr.protocol.did_reach_tentative_stage:
                        ...
                        self.logger.error("Shuffle of coin {} did reach the 'tentative' stage. Will unreserve its reserved addresses in {} minutes."
            ...
            self.logger.error("No sender! Thr={}".format(str(thr)))
        if thr is self.threads[scale]:
            self.threads[scale] = None
        elif thr.is_alive():
            self.logger.error("WARNING: Stopping thread ({}) which was not in the self.threads dict for scale = {} coin = {}"
                             .format(str(thr), scale, sender))
        if thr.is_alive():
            thr.join()
        else:
            thr.stop()
            self.logger.error("Thread already exited; cleaned up.")
        return retVal
    ...
    def protocol_thread_callback(self, thr, message):
        ...
        def signal_stop_thread(thr, message):
            ...
            self.logger.error("Signalling stop for scale: {}".format(thr.scale))
            self.shared_chan.send((True, thr, message))
        def fwd_message(thr, message):
            #self.logger.error("Fwd msg for: Scale='{}' Msg='{}'".format(thr.scale, message))
            self.shared_chan.send((False, thr, message))
        scale = thr.scale
        thr.ts = time.time()
        self.logger.error("Scale: {} Message: '{}'".format(scale, message.strip()))
    ...
    def _make_protocol_thread(self, scale, coins, scale_lower_bound, scale_upper_bound):
        ...
        self.logger.error("Scale {} Coin {} OutAddr {} {} {} make_protocol_thread".format(scale, utxo_name, output.to_storage_string(), "Change" if may_receive_change else "FakeChange", change.to_storage_string()))
        #self.logger.error("Reserved addresses:", self.wallet._addresses_cashshuffle_reserved)
        ...
        with self.wallet.lock:
            if self.is_wallet_ready():
                try:
                    ...
                    self.logger.error("check_for_coins error: {}".format(str(e)))
        if need_refresh:
    ...
    def join(self):
        ...
        if self.is_alive():
            self.logger.error("Joining still-running thread...")
            super().join()
