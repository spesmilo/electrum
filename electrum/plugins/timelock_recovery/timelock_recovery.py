...
class TimelockRecoveryPlugin(BasePlugin):
    ...
    def get_noise_map(cls, versioned_seed: VersionedSeed) -> Dict[Tuple[int, int], int]:
        ...
        if version == '0':
            ...
            raise Exception(f"unexpected timelock recovery version: {version}")
        return noise_map
...
if __name__ == '__main__':
    for i in range(10**4):
        vs = TimelockRecoveryPlugin.gen_random_versioned_seed()
        nm = TimelockRecoveryPlugin.get_noise_map(vs)
