...
class NWCServerPlugin(BasePlugin):
    ...
    def __init__(self, parent, config: 'SimpleConfig', name):
        ...
        self.config = config
        self.connections = None # type: Optional[dict[str, dict]]  # hex pubkey -> connection data
        self.budgets = None  # type: Optional[dict[str, List[List[int]]]]  # pubkey_hex -> [[amount, timestamp]]
...
class NWCServer(Logger, EventListener):
    ...
    def __init__(self, config: 'SimpleConfig', wallet: 'Abstract_Wallet', taskgroup: 'OldTaskGroup'):
        ...
        self.manager = self.get_relay_manager()  # type: aionostr.Manager
        ...
        self.budgets = storage['budgets']  # type: dict[str, List[List[int]]]  # pubkey_hex -> [[amount, timestamp]]
    ...
    async def run(self) -> None:
        while True:
            ...
            # connect if we are not yet connected or need to reconnect
            if len(self.manager.relays) <= 0:  # setup new manager so relays are populated again
                self.manager = self.get_relay_manager()
            if not self.manager.connected:
                await self.manager.connect()
            if len(self.manager.relays) <= 0:  # manager should have relays after connecting
            ...
            try:
                ...
                self.logger.exception("Restarting nwc event handler")
    ...
    async def handle_requests(self) -> None:
        ...
        async for event in self.manager.get_events(query, single_event=False, only_stored=False):
            ...
            if valid_until and valid_until <= int(time.time()):
                ...
                self.budgets.pop(event.pubkey, None)
    ...
    async def pay_invoice(self, b11: str, amount_msat: Optional[int], request_pub: str) -> dict:
        ...
        if not self.add_to_budget(request_pub, invoice.get_amount_sat()):
    ...
    def add_to_budget(self, client_pub: str, amount_sat: int) -> bool:
        """
        ...
        if client_pub not in self.budgets:
            self.budgets[client_pub] = []
        used_budget: int = self.get_used_budget(client_pub)
        if used_budget + amount_sat > client_budget_sat:
            return False
        # tuples don't work because jsondb converts them to lists on reload
        self.budgets[client_pub].append([amount_sat, int(time.time())])
        return True
    ...
    def get_used_budget(self, client_pub: str) -> int:
        ...
        if client_pub not in self.budgets:
            return 0
        used_budget: int = 0
        for amount, timestamp in list(self.budgets[client_pub]):
            if timestamp > int(time.time()) - 24 * 3600:
                used_budget += amount
            elif timestamp < int(time.time()) - 24 * 3600:
                # remove old expense
                try:
                    self.budgets[client_pub].remove([amount, timestamp])
                except ValueError:
