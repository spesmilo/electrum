...
from PyQt6.QtWidgets import QVBoxLayout, QPushButton, QLabel, QTreeWidget, QTreeWidgetItem, \
    QTextEdit, QApplication, QSpinBox, QSizePolicy, QComboBox
...
class Plugin(NWCServerPlugin):
    ...
    def settings_dialog(self, window: WindowModalDialog):
        ...
        connections_list.setHeaderLabels([_("Name"), _("Budget"), _("Expiry")])
        ...
        def update_connections_list():
            ...
            for name, conn in connections.items():
                ...
                if conn['daily_limit_sat'] == 'unset':
                    ...
                    budget = str(conn['daily_limit_sat'])
                    used = str(self.nwc_server.get_used_budget(conn['client_pub']))
                    limit = f"{used}/{budget}"
    ...
    def connection_info_input_dialog(self, window) -> Optional[str]:
        ...
        limit_edit = QSpinBox()
        limit_edit.setRange(0, 100_000_000)
        ...
        validity_edit = QSpinBox()
        validity_edit.setRange(0, 63072000)
        ...
        if not name or len(name) < 1:
            ...
            return None
        value_limit = limit_edit.value() if limit_edit.value() > 0 else None
        duration_limit = validity_edit.value() if validity_edit.value() > 0 else None
    ...
    def show_new_connection_dialog(window, connection_string: str):
        ...
        vbox.addLayout(Buttons(copy_btn, OkButton(popup)))
