...
class TxEngine(Logger):
    ...
    def add_future_tx(self, sweep_info, wanted_height):
        ...
        prevout = txin.prevout.to_str()
        prev_txid, index = prevout.split(':')
        if self.wallet.adb.db.get_spent_outpoint(prev_txid, int(index)):
            return
        name = sweep_info.name
        prevout = txin.prevout.to_str()
        new_tx = self.wallet.create_transaction(
            inputs=[txin],
            outputs=[],
            password=None,
            fee=0,
        ...
        try:
            tx_was_added = self.wallet.adb.add_transaction(new_tx)#, is_new=(old_tx is None))
        except Exception as e:
        ...
        self.wallet.adb.set_future_tx(new_tx.txid(), wanted_height=wanted_height)
        if tx_was_added:
