...
from .peerbackup import PeerBackup, PEERBACKUP_VERSION, deserialize_inactive_htlcs, hash_htlc_history
...
class Peer(Logger, EventListener):
    ...
    def on_query_htlc_history(self, chan, payload):
        ...
        htlc_history_local, local_first_hash, local_delta_msat = chan.hm.get_htlc_history(-proposer, LOCAL, index)
        htlc_history_remote, remote_first_hash, remote_delta_msat = chan.hm.get_htlc_history(-proposer, REMOTE, index)

        assert htlc_history_local == htlc_history_remote # merge them
        htlc_history = htlc_history_local
        ...
        for htlc_update in htlc_history:
            htlc_update.flip()
            _bytes = htlc_update.to_bytes(owner=None, blank_timestamps=False)
