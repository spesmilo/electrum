...
class Peer(Logger, EventListener):
    ...
    def on_query_htlc_history(self, chan, payload):
        ...
        local_first_hash, local_delta_msat, _ = chan.hm.get_checkpoint(REMOTE, -proposer, LOCAL, checkpoint)
        remote_first_hash, remote_delta_msat, _ = chan.hm.get_checkpoint(REMOTE, -proposer, REMOTE, checkpoint)
    ...
    async def sync_htlc_history(self, proposer):
        if os.environ.get('PEERBACKUPS_DONT_SYNC_HTLCS'):
        ...
        while True:
            ...
            for chan in self.channels.values():
                if chan.get_state() != ChannelState.OPEN:
                    continue
                local_cp = chan.hm.get_root_checkpoint(proposer, LOCAL)
                remote_cp = chan.hm.get_root_checkpoint(proposer, REMOTE)
                target_cp = max(local_cp, remote_cp)
                if target_cp == -1:
                    continue
                ...
                self.logger.info(f'sync_htlc_history: {proposer.name}: {local_cp=} {remote_cp=}')
                ...
                old_local_hash, local_delta_msat  = chan.hm.hash_htlc_history(
                    local_htlc_history[0:(local_cp - target_cp) or None],
                    role=LOCAL,
                    proposer=proposer,
                    owner=LOCAL,
                    first_hash=local_first_hash)
                ...
                old_remote_hash, remote_delta_msat = chan.hm.hash_htlc_history(
                    remote_htlc_history[0:(remote_cp - target_cp) or None],
                    role=LOCAL,
                    proposer=proposer,
                    owner=REMOTE,
                    first_hash=remote_first_hash)
                # check that it connects to our checkpoint
                expected_local_hash, expected_local_delta, _ = chan.hm.get_checkpoint(LOCAL, proposer, LOCAL, local_cp)
                expected_remote_hash, expected_remote_delta, _ = chan.hm.get_checkpoint(LOCAL, proposer, REMOTE, remote_cp)
                ...
                htlc_history = deserialize_inactive_htlcs(htlc_history_bytes, owner=None)
                chan.hm.update_htlc_history(proposer, new_cp + 1, htlc_history)
                new_local_delta = expected_local_delta - local_delta_msat
                new_remote_delta = expected_remote_delta - remote_delta_msat
                # save new checkpoint
                if new_cp < local_cp:
                    chan.hm.save_checkpoint(proposer, LOCAL, local_cp, old_local_hash, expected_local_delta, False)
                    chan.hm.save_checkpoint(proposer, LOCAL, new_cp, local_first_hash, new_local_delta, True)
                if new_cp < remote_cp:
                    chan.hm.save_checkpoint(proposer, REMOTE, remote_cp, old_remote_hash, expected_remote_delta, False)
                    chan.hm.save_checkpoint(proposer, REMOTE, new_cp, remote_first_hash, new_remote_delta, True)
    ...
    def is_peerbackup_client(self):
        return self.lnworker.features.supports(LnFeatures.OPTION_ELECTRUM_PEERBACKUP_CLIENT_OPT)\
            and self.their_features.supports(LnFeatures.OPTION_ELECTRUM_PEERBACKUP_SERVER_OPT)

    def is_peerbackup_server(self):
        return self.lnworker.features.supports(LnFeatures.OPTION_ELECTRUM_PEERBACKUP_SERVER_OPT)\
    ...
    async def channel_establishment_flow(
        ...
        # fixme: peerbackup should be able to derive this
        if not self.is_peerbackup_client() and our_channel_type & ChannelType.OPTION_ANCHORS_ZERO_FEE_HTLC_TX:
    ...
    async def on_open_channel(self, payload):
        ...
        if not self.is_peerbackup_client() and channel_type & ChannelType.OPTION_ANCHORS_ZERO_FEE_HTLC_TX:
    ...
    def recreate_channel(self, peerbackup_tlvs: dict, old_chan):
        ...
        self.logger.info(f'rev_then_sig {chan.hm.is_expecting_rev()}')
        self.lnworker.add_new_channel(chan)
        return chan
    ...
    def _send_channel_reestablish(self, chan: Channel):
        ...
        if self.is_peerbackup_server():
            if peerbackup_bytes := chan.get_their_peerbackup_for_reestablish():
                local_ctn, local_timestamp, local_signature = chan.get_time_commitment(LOCAL)
                remote_ctn, remote_timestamp, remote_signature = chan.get_time_commitment(REMOTE)
                channel_reestablish_tlvs['peerbackup'] = {
    ...
    def on_commitment_signed(self, chan: Channel, payload) -> None:
        ...
        if self.is_peerbackup_server():
            ...
            chan.receive_peerbackup_signature(peerbackup_sig, REMOTE)
        if self.is_peerbackup_client():
    ...
    def on_revoke_and_ack(self, chan: Channel, payload) -> None:
        ...
        if self.is_peerbackup_server():
            ...
            chan.receive_peerbackup_signature(peerbackup_sig, LOCAL)
        if self.is_peerbackup_client():
