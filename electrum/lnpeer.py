...
class Peer(Logger, EventListener):
    ...
    async def on_open_channel(self, payload: dict):
        try:
            ...
            self.logger.warning(f"rejecting incoming channel: {e}")
            self.send_warning(payload["temporary_channel_id"], message="rate limited, try again later", close_connection=False)
