...
def sweep_their_ctx_watchtower(
        chan: 'Channel',
        ctx: Transaction, per_commitment_secret: bytes,
        sweep_address: str) -> List[PartialTxInput]:
    ...
    if output_idxs:
        ...
        if txin:
            ...
            tx_size_bytes = 121
            fee = chan.lnworker.config.estimate_fee(tx_size_bytes, allow_fallback_to_static_rates=True)
            outvalue = val - fee
            ...
            #chan.lnworker.wallet.sign_transaction(sweep_tx, None)
...
def sweep_their_ctx_htlc(
        ctx: Transaction, witness_script: bytes,
...
def sweep_their_ctx_to_remote(
    ...
    if not has_anchors:
        ...
        txin.make_witness = lambda sig: construct_witness([sig, txin.witness_script])
...
def sweep_ctx_to_local(
    ...
    txin.privkey = privkey
    assert txin.witness_script
    txin.make_witness = lambda sig: construct_witness([sig, int(is_revocation), txin.witness_script])
    return txin
