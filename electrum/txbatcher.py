...
from .util import log_exceptions, NotEnoughFunds
...
class TxBatcher(Logger):
    ...
    def _maybe_create_new_batch(self, key, fee_policy):
        descriptor = fee_policy.get_descriptor()
        if key not in self.storage:
            self.storage[key] = { 'fee_policy': descriptor, 'txids': [] }
            self.tx_batches[key] = TxBatch(self.wallet, self.storage[key])
        elif self.storage[key]['fee_policy'] != descriptor:
