...
class TxBatcher(Logger):
    ...
    @locked
    def add_payment_output(self, key: str, output: 'PartialTxOutput', fee_policy):
        batch = self._maybe_create_new_batch(key, fee_policy)
        batch.add_payment_output(output)

    @locked
    def add_sweep_input(self, key: str, sweep_info: 'SweepInfo', fee_policy):
        batch = self._maybe_create_new_batch(key, fee_policy)
        batch.add_sweep_input(sweep_info)

    def _maybe_create_new_batch(self, key, fee_policy):
        if key not in self.storage:
            self.storage[key] = { 'fee_policy': fee_policy.get_descriptor(), 'txids': [] }
            self.tx_batches[key] = TxBatch(self.wallet, self.storage[key])
        return self.tx_batches[key]

    def _delete_batch(self, key):
        self.logger.info(f'destroying batch {key}')
...
class TxBatch(Logger):
    ...
    def is_done(self):
        ...
        return len(self.batch_inputs) == 0 and len(self.batch_payments) == 0 and self._find_confirmed_base_tx()
