...
class ElectrumBatchClient(ElectrumClient):
    ...
    async def get_balances(self, script_hashes: List[str], *args, **kwargs) -> dict:
        self.logger.info(
            f"start get_balances in thread #{kwargs.get('thread_numb', 0)}, count={len(script_hashes)}, batch_lim={self.batch_limit}")
        ...
        try:
            ...
            self.logger.info(
                f"stop get_balances in thread #{kwargs.get('thread_numb', 0)}. count of processed addresses is {count}")
            return result

    async def get_listunspents(self, script_hashes: List[str], *args, **kwargs) -> dict:
        self.logger.info(
            f"start get_listunspents in thread #{kwargs.get('thread_numb', 0)}, count={len(script_hashes)}, batch_lim={self.batch_limit} ---- {script_hashes[0]}")
        count = 0
        result = {}
        try:
            for shs in self.chunks(script_hashes, self.batch_limit):
            ...
            self.logger.info(
                f"stop get_listunspents in thread #{kwargs.get('thread_numb', 0)}. count of processed addresses is {count},")
            return result

    async def get_listmempools(self, script_hashes: List[str], *args, **kwargs) -> dict:
        self.logger.info(
            f"start listmempools_for_scripthashes in thread #{kwargs.get('thread_numb', 0)}, count={len(script_hashes)}, batch_lim={self.batch_limit} ---- {script_hashes[0]}")
        count = 0
        result = {}
        try:
            for shs in self.chunks(script_hashes, self.batch_limit):
            ...
            self.logger.info(
                f"stop listmempools_for_scripthashes in thread #{kwargs.get('thread_numb', 0)}. count of processed addresses is {count},")
            return result
