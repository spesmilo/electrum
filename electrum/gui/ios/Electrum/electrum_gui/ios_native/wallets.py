...
def StatusColors() -> dict:
    return {
        ...
        StatusDownloadingHeaders:
            UIColor.colorWithRed_green_blue_alpha_(255.0 / 255.0, 194.0 / 255.0, 104.0 / 255.0, 1.0),
...
class WalletsVC(WalletsVCBase):
    ...
    def setStatus_(self, mode: int) -> None:
        ...
        if mode == StatusOnline:
            ...
            self.statusLabel.text = " " + _(
                "Downloading Headers") + "   "  # hack -- pad with spaces so it look right.. TODO: fix this issue
    ...
    def toggleDrawer(self) -> None:
        ...
        if not self.modalDrawerVC:
            ...
            if not vc:
                utils.NSLog(
                    "**** Warning: toggleDrawer could not find the requisite view controller in WalletsDrawerVC.nib!")
                return
            ...
            def compl() -> None:
                vc.view.backgroundColorAnimationToColor_duration_reverses_completion_(semiclear.autorelease(), 0.2,
                                                                                      False, None)
    ...
    def onTopNavTap(self) -> None:
        ...
        self.statusLabel.backgroundColorAnimationToColor_duration_reverses_completion_(c1, 0.2, True,
                                                                                       doShowNetworkDialog)
        ...
        if self.tv:
            ...
            for i, wallet in enumerate(wallets):
                if current == wallet.name:
                    self.tv.scrollToRowAtIndexPath_atScrollPosition_animated_(
                        NSIndexPath.indexPathForRow_inSection_(i, 0), UITableViewScrollPositionMiddle, False)
                    return
                ...
                if info.size:
                    def DoIt() -> None:
                        if self and self.ptr.value and self.viewIfLoaded and self.viewIfLoaded.window:
                            _ShowOptionsForWalletAtIndex(vc=self, index=row,
                                                         ipadAnchor=cell.convertRect_toView_(cell.bounds, self.view))
    ...
    def DoDelete() -> None:
        ...
        if isCurrent:
            utils.show_alert(vc=vc,
                             ...
                             message=_(
                                 "You are requesting the deletion of the currently active wallet. In order to delete this wallet, please switch to another wallet, then select this option again on this wallet."),
                             actions=[[_("OK")]])
            return
        ...
        def DeleteChk() -> None:
            ...
            if tf:
                ...
                if txt == 'delete':
                    ...
                    parent.show_error(vc=vc, title=_("Not Deleted"), message=_(
                        "You didn't enter the text 'delete' in the previous dialog. For your own safety, the wallet file was not deleted."))
            Release()
        ...
        utils.show_alert(vc=vc,
                         ...
                         message=_(
                             "You are about to delete the wallet '{}'. Unless you have other copies of this wallet or you wrote its seed down, you may lose funds!\n\nIn order to proceed, please type the word 'delete' in the box below:").format(
                             info.name),
                         actions=[[_("Cancel"), Release], [_("Delete"), DeleteChk]], cancel=_("Cancel"),
