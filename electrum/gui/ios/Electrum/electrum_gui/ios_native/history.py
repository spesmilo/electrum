...
HistoryEntry = namedtuple("HistoryEntry",
                          "tx tx_hash status_str label v_str balance_str date ts conf status value fiat_amount fiat_balance fiat_amount_str fiat_balance_str ccy status_image")
#######################################################################
...
def get_history(domain: list = None, statusImagesOverride: list = None, forceNoFX: bool = False) -> list:
    """ For a given set of addresses (or None for all addresses), builds a list of
        HistoryEntry """
    sImages = StatusImages if not statusImagesOverride or len(statusImagesOverride) < len(
        StatusImages) else statusImagesOverride
    ...
    for h_item in h:
        ...
        date = timestamp_to_datetime(time.time() if mined_status.conf <= 0 else mined_status.timestamp)
        ...
        if not forceNoFX and fx:
            if not ccy:
                ccy = fx.get_currency()
            try:
                hdate = timestamp_to_datetime(time.time() if mined_status.conf <= 0 else mined_status.timestamp)
        ...
        entry = HistoryEntry(tx, tx_hash, status_str, label, v_str, balance_str, date, ts, mined_status.conf, status, value,
                             fiat_amount, fiat_balance, fiat_amount_str, fiat_balance_str, ccy, img)
    ...
    def doReloadForKey(self, key: Any) -> Any:
        ...
        # contacts entires store history entries within themselves.. so just return that
            ...
            hist = contacts.build_contact_tx_list(
                key.address)  # force refresh of tx's from wallet -- this will call us again with 'None'
        elif isinstance(key, str):
            # support for string addresses or tx_hashes.. detect which and act accordingly
            if is_address(key):
                hist = self.get([key])  # recursively call self with propery list data type, which will end up calling get_history (it's ok -- this is to cache results uniformly!)
                duped = ' (duped) '
        ...
        if unk:
            utils.NSLog("HistoryMgr: failed to retrieve any data for unknown domain=%s, returning empty list",
                        dstr[:80])
        else:
            utils.NSLog("HistoryMgr: refresh %d entries for domain=%s in %f ms%s", len(hist), dstr[:80],
                        (time.time() - t0) * 1e3, duped)
        return hist
...
class TxHistoryHelper(TxHistoryHelperBase):
    ...
    def tableView_cellForRowAtIndexPath_(self, tableView, indexPath) -> ObjCInstance:
        ...
        if (self.compactMode and not _is_ipad) or (not entry.fiat_amount_str and not entry.fiat_balance_str):
            ...
            cell.amount.attributedText = utils.hackyFiatAmtAttrStr(amtStr, utils.stripAmount(entry.fiat_amount_str),
                                                                   entry.ccy, s2.width - s1.width,
                                                                   utils.uicolor_custom('light'), adjustCS,
                                                                   utils._kern * 1.25, isIpad=_is_ipad)
            cell.balance.attributedText = utils.hackyFiatAmtAttrStr(balStr, utils.stripAmount(entry.fiat_balance_str),
                                                                    entry.ccy, s1.width - s2.width,
                                                                    utils.uicolor_custom('light'), adjustCS,
                                                                    utils._kern * 1.25, isIpad=_is_ipad)
            # end experimental zone...
        cell.desc.setText_withKerning_(entry.label.strip() if isinstance(entry.label, str) else '', utils._kern)
        cell.icon.image = UIImage.imageNamed_(
            "tx_send.png") if entry.value and entry.value < 0 else UIImage.imageNamed_("tx_recv.png")
        if entry.conf > 0:
...
def NewTxHistoryHelper(tv: ObjCInstance, vc: ObjCInstance, domain: list = None, noRefreshControl=False,
