...
from . import utils
from . import gui
from electrum.util import block_explorer_info, base_units, block_explorer
...
class PrefsVC(UITableViewController):
    ...
    def updateCurrencies(self):
        ...
        currencies = [self.currencies[0],
                      *sorted(parent.daemon.fx.get_currencies(parent.daemon.fx.get_history_config()))]
    ...
    def setupCell_section_row_(self, cell: ObjCInstance, secName_oc: ObjCInstance, row: int) -> None:
        ...
        if secName == 'Tools':
            ...
            if row == 0:  # 'edit fees manually', a bool cell
                ...
                s = cell.viewWithTag_(2)
                if s.allTargets.count <= 0:
                    s.addTarget_action_forControlEvents_(self, SEL(b'onUseRBF:'), UIControlEventValueChanged)
                l.text = _("Use Replace-By-Fee")
                s.on = parent.prefs_get_use_rbf()
            elif row == 2:
                l = cell.viewWithTag_(1)
                b = cell.viewWithTag_(2)
                b = b if b is not None else cell.viewWithTag_(TAG_FEE_ESTIMATION)
                l.text = _('Fee estimation')
                if b is not None:
                    b.tag = TAG_FEE_ESTIMATION
                    choices, current_choice = parent.get_fee_estimation()
                    b.setTitle_forState_(choices[current_choice], UIControlStateNormal)
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onFeeEstimationBut:'),
            ...
            if row == 0:
                ...
                if b is not None:
                    b.tag = TAG_NZ
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onNZBut:'),
                                                             UIControlEventPrimaryActionTriggered)
                ...
                if b is not None:
                    ...
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onBaseUnitBut:'),
                                                             UIControlEventPrimaryActionTriggered)
            elif row == 3:
                l = cell.viewWithTag_(1)
                b = cell.viewWithTag_(2)
                ...
                if b is not None:
                    ...
                    be = list(block_explorer_info().keys())
                    be = be if be is not None and len(be) > 0 else ["None"]
                    beprefs = block_explorer(parent.config)
                    if beprefs not in be:  beprefs = be[0]
                    b.setTitle_forState_(beprefs, UIControlStateNormal)
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onBlockExplorerBut:'),
                                                             UIControlEventPrimaryActionTriggered)
        elif secName == 'Fiat':
            if row == 0:
                l = cell.viewWithTag_(1)
                ...
                if b is not None:
                    ...
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onFiatCurrencyBut:'),
                                                             UIControlEventPrimaryActionTriggered)
            elif row == 1:
                l = cell.viewWithTag_(1)
                s = cell.viewWithTag_(2)
                ...
                if b is not None:
                    ...
                    if b.allTargets.count <= 0:
                        b.addTarget_action_forControlEvents_(self, SEL(b'onFiatExchangeBut:'),
                                                             UIControlEventPrimaryActionTriggered)
    ...
    def createCellForSection_row_(self, secName_oc: ObjCInstance, row: int) -> ObjCInstance:
        ...
        if secName in ['Tools']:
            cell = UITableViewCell.alloc().initWithStyle_reuseIdentifier_(UITableViewCellStyleSubtitle,
                                                                          ident).autorelease()
        elif ident in ['Fees_0', 'Fees_1', 'Transactions_0', 'Transactions_1', 'Transactions_2', 'Appearance_0',
                       'Fiat_1', 'Fiat_2']:
            objs = NSBundle.mainBundle.loadNibNamed_owner_options_("BoolCell", self.tableView, None)
            assert objs is not None and len(objs)
            cell = objs[0]
        elif ident in ['Fees_2', 'Appearance_1', 'Appearance_2', 'Appearance_3', 'Fiat_0', 'Fiat_3']:
    ...
    @objc_method
    def onFeeEstimationBut_(self, button: ObjCInstance):
        parent = gui.ElectrumGui.gui
        choices, current_choice = parent.get_fee_estimation()

        def onOk(selIdx: int) -> None:
            parent.prefs_set_fee_estimation(selIdx)
            b = self.view.viewWithTag_(TAG_FEE_ESTIMATION)
            choices, current_choice = parent.get_fee_estimation()
            if b is not None:
                b.setTitle_forState_(str(choices[current_choice]), UIControlStateNormal)

        utils.present_modal_picker(parentVC=self, items=choices, selectedIndex=current_choice,
    ...
    @objc_method
    def onUseRBF_(self, switch: ObjCInstance) -> None:
        parent = gui.ElectrumGui.gui
        parent.prefs_set_use_rbf(switch.isOn())
