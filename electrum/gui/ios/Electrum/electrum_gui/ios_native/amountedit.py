...
from electrum.util import format_satoshis_plain, format_satoshis
...
class FiatAmountEdit(BTCAmountEdit):
    ...
    def baseUnit(self) -> ObjCInstance:
        return ns_from_py(
            parent().daemon.fx.get_currency() if parent().daemon and parent().daemon.fx and parent().daemon.fx.is_enabled() else "USD")
