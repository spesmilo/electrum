...
CoinsEntry = namedtuple("CoinsEntry",
                        "utxo tx_hash address address_str height name label amount amount_str is_frozen is_change base_unit")
...
class CoinsDetail(CoinsDetailBase):
    ...
    def refresh(self) -> None:
        ...
        if not coin:
            ...
            utils.NSLog(
                "WARNING: CoinsDetailVC lost its 'coin' entry from underneath its own feet. This could mean the user spent the coin on another screen or something else strange happened. Backing out!")
            return

        self.address.text = coin.address
    ...
    def viewWillTransitionToSize_withTransitionCoordinator_(self, size: CGSize, coordinator: ObjCInstance) -> None:
        send_super(__class__, self, 'viewWillTransitionToSize:withTransitionCoordinator:', size, coordinator.ptr,
...
class CoinsTableVC(UITableViewController):
    ...
    def initWithStyle_(self, style: int) -> ObjCInstance:
        ...
        buts = [
            UIBarButtonItem.alloc().initWithTitle_style_target_action_(_("Spend"), UIBarButtonItemStyleDone, self,
                                                                       SEL(b'spendFromSelection')).autorelease(),
            UIBarButtonItem.alloc().initWithTitle_style_target_action_(_("Clear"), UIBarButtonItemStylePlain, self,
                                                                       SEL(b'clearSelection')).autorelease(),
        ]
    ...
    def viewDidLoad(self) -> None:
        ...
        if objs:
            ...
            if lbl: lbl.attributedText = utils.ats_replace_font(lbl.attributedText,
                                                                UIFont.italicSystemFontOfSize_(14.0))
        else:
            NSLog("WARNING: Could not find the 'no coins' view in Misc.xib!")
            ...
            if coins and isGood:
                ...
                def doDetail(acell: objc_id) -> None:
                    ...
                    acell.accessoryFlashView.backgroundColorAnimationFromColor_toColor_duration_reverses_completion_(
                        UIColor.colorWithRed_green_blue_alpha_(0.5, 0.5, 0.5, 0.4), UIColor.clearColor, animDur, False,
                        None
                    )
    ...
    @objc_method
    def selectDeselectCell_(self,
                            cell: ObjCInstance) -> bool:  # returns False IFF it was a frozen address and select/deselect failed
        coins = _Get(self)
        if not coins or not len(coins): return True

        index = cell.tag
        self.setIndex_selected_(index, not self.isIndexSelected_(index))
        wasSel = self.setupSelectionButtonCell_atIndex_(cell,
                                                        index)  # this sometimes fails if address is frozen and/or we are watching only
    ...
    def setupSelectionButtonCell_atIndex_(self, cell, index: int) -> bool:
        if not isinstance(cell, CoinsCell):
            utils.NSLog(
                "*** WARNING: setupSelectionButtonCell_atIndex_ called with an unknown cell type! Returning early...")
            return False
        parent = gui.ElectrumGui.gui
...
class CoinsMgr(utils.DataMgr):
    def doReloadForKey(self, key: Any) -> Any:
        t0 = time.time()
        c = get_coins(key)
        utils.NSLog("CoinsMgr: Fetched %d utxo entries [domain=%s] in %f ms", len(c), str(key)[:16],
                    (time.time() - t0) * 1e3)
        return c


def get_coin_counts(domain: list, exclude_frozen: bool = False, mature: bool = False,
                    confirmed_only: bool = False) -> int:
...
def get_coins(domain: list = None, exclude_frozen: bool = False, mature: bool = False,
    ...
    for x in c:
        ...
        entry = CoinsEntry(x, tx_hash, address, address_str, height, name, label, amount, amount_str, is_frozen,
                           is_change, base_unit)
        coins.append(entry)
