...
from electrum.bitcoin import b58_address_to_hash160, is_segwit_address
from . import utils
from . import gui
...
class AddressDetail(AddressDetailBase):
    ...
    def init(self) -> ObjCInstance:
        raise ValueError(
            "INVALID USAGE: Cannot construct an AddressDetail with a simple 'init' call! Use 'initWithAddress:' instead ")
    ...
    def initWithAddress_(self, address) -> ObjCInstance:
        ...
        if self:
            ...
            self.domain = address
            self.optionsBarBut = UIBarButtonItem.alloc().initWithImage_style_target_action_(
    ...
    def loadView(self) -> None:
        ...
        helper = history.NewTxHistoryHelper(tv=self.tv, vc=self, noRefreshControl=True, domain=[entry.address],
                                            cls=history.TxHistoryHelperWithHeader)
    ...
    def viewDidLoad(self) -> None:
        ...
        def didEndEditing(text: ObjCInstance) -> None:
            ...
            text = str(text).strip()
            new_label = text
            print("new label for address %s = %s" % (entry.address, new_label))
    ...
    def refresh(self) -> None:
        ...
        self.address.text = entry.address
        ...
        def numTXsAttrStr() -> ObjCInstance:
            ...
            ats.appendAttributedString_(
                NSAttributedString.alloc().initWithString_attributes_(utxos, attrs).autorelease())
...
class AddressesVC(AddressesVCBase):
    ...
    def tableView_numberOfRowsInSection_(self, tableView: ObjCInstance, section: int) -> int:
        try:
            addrData = _Get()
            return max(1, len(
                addrData.master[self.comboL.selection][self.comboR.selection])) if addrData is not None else 0
        except:
    ...
    def tableView_cellForRowAtIndexPath_(self, tableView, indexPath):
        ...
        if self.mode == ModeNormal:
            ...
            cell.balance.attributedText = utils.hackyFiatAmtAttrStr(baltxt, entry.fiat_balance_str, '', 0.0, fiatColor,
                                                                    None, -0.5, amtColor=amtColor)
            xtra = []
            if entry.is_frozen:
                if entry.is_change:
                    xtra += [_('Change')]
            ...
            cell.address.tag = (self.comboL.selection << 24) | (self.comboR.selection << 16) | (
                        indexPath.row & 0xffff)  # useful for onTapAddress to figure out what tapped it
            ...
            cell.detailTextLabel.text = "bal: " + entry.balance_str + (
                (' (' + entry.fiat_balance_str + ')') if addrData.show_fx else '') + " numtx: " + str(entry.num_tx) + (
    ...
    def setupComboCallbacks(self) -> None:
        ...
        def bgTapChk(p: CGPoint) -> None:
            ...
            if isinstance(this, ComboDrawerPicker):
                ...
                that = None
                if CGRectContainsPoint(fwl, p):
                    that = self.comboL
                elif CGRectContainsPoint(fwr, p):
                    that = self.comboR
                if that:
                    ...
                    hax.opaque = False
                    hax2 = UIView.alloc().initWithFrame_(
                        this.bottomView.convertRect_toView_(this.bottomView.bounds, None)).autorelease()
    ...
    def doComboOpen_(self, vc) -> None:
        ...
        def compl() -> None:
            vc.view.backgroundColorAnimationToColor_duration_reverses_completion_(semiclear.autorelease(), 0.2, False,
                                                                                  None)
...
class AddressData:
    Entry = namedtuple("Entry",
                       "address addr_str addr_idx label balance_str fiat_balance_str num_tx is_frozen balance is_change is_used base_unit is_watch_only num_utxos")
    ...
    def refresh(self):
        ...
        for is_change in sequences:
            ...
            for n, address in enumerate(addr_list):
                numAddresses += 1
                num = wallet.get_address_history_len(address)
                ...
                item = AddressData.Entry(address, address_text, n, label, balance_text, fiat_balance, num,
                                         ...
                                         is_watch_only, num_utxos)

                # _TYPES = ("Any","Receiving","Change")
                # _STATUSES = ("All", "Funded", "Unused", "Used")
                self.master[0][0].append(item)  # item belongs in 'Any,All' regardless
                self.master[2 if item.is_change else 1][0].append(
                    item)  # append to either change or receiving of 'All' list
                if item.balance:
                    self.master[0][1].append(item)  # item belongs in 'Any,Funded' regardless
                    self.master[2 if item.is_change else 1][1].append(
                        item)  # append to either change or receiving of 'Funded' list
                if item.is_used:
                    self.master[0][3].append(item)  # item belongs in the 'Any,Used' always, if used
                    self.master[2 if item.is_change else 1][3].append(
                        item)  # append to either change or receiving of 'All' list
                else:  # Unused list
                    self.master[0][2].append(item)  # item belongs in the 'Any,Unused' always, if unused
                    self.master[2 if item.is_change else 1][2].append(
                        item)  # append to either change or receiving of 'All' list
...
def PushDetail(address_or_entry: object, navController: ObjCInstance) -> ObjCInstance:
    entry = None
    if isinstance(address_or_entry, str):
        entry = EntryForAddress(address_or_entry)
    ...
    addrDetail = AddressDetail.alloc().initWithAddress_(entry.address).autorelease()
