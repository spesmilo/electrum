...
class QEAmount(QObject):
    ...
    def setToMax(self, one: 'QEAmount|None', two: 'QEAmount|None'):
        """assign max(one, two) to self.
           note this does not return a new QEAmount instance. QEAmount is implemented in python,
           so an instance handed to QML needs a strong reference on the python side to stay alive,
           which would either accumulate on the owner or leave QML with a deleted object.
           """
        if one is None:
            one = QEAmount()
        if two is None:
        ...
        if one.isMax:
            ...
            self.copyFrom(one if one.gt(two) else two)

    @pyqtSlot('QVariant', 'QVariant')
    def setToMin(self, one: 'QEAmount|None', two: 'QEAmount|None'):
        """assign min(one, two) to self. see setToMax"""
        if one is None:
            one = QEAmount()
        if two is None:
            two = QEAmount()
