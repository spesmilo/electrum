...
class AmountEdit(SizedFreezableLineEdit):
    ...
    def get_amount(self) -> Union[None, Decimal, int]:
        ...
        if amt is None:
            return amt
        # return as int if no fractional precision (Decimal otherwise)
        # cut-off at extra_precision()
        scale_factor = pow(10, self.extra_precision())
        return int(amt) if (int(amt) * scale_factor == int(amt * scale_factor)) else amt
