...
class TestConfig(QETestCase):
    ...
    @qt_test
    def test_amounttobaseunitstr(self):
        self.q.config.BTC_AMOUNTS_DECIMAL_POINT = 5
        self.assertEqual(self.q.amountToBaseunitStr(100_000), '1')
        self.assertEqual(self.q.amountToBaseunitStr(1), '0.00001')
        amt = QEAmount(amount_msat=1)
        self.assertEqual(self.q.amountToBaseunitStr(amt), '0.00000001')

    @qt_test
    def test_baseunitstrtoamount(self):
        qa = self.q.baseunitStrToAmount('')
        self.assertTrue(qa.isEmpty)
        qa = self.q.baseunitStrToAmount('0')
        self.assertTrue(qa.isEmpty)
        qa = self.q.baseunitStrToAmount('0.000')
        self.assertTrue(qa.isEmpty)

        self.q.config.BTC_AMOUNTS_DECIMAL_POINT = 5

        qa = self.q.baseunitStrToAmount('1')
        self.assertFalse(qa.isEmpty)
        self.assertEqual(qa.satsInt, 100_000)
        self.assertEqual(qa.msatsInt, 100_000_000)

        qa = self.q.baseunitStrToAmount('1.001')
        self.assertFalse(qa.isEmpty)
        self.assertEqual(qa.satsInt, 100_100)
        self.assertEqual(qa.msatsInt, 100_100_000)

        qa = self.q.baseunitStrToAmount('1.000001')
        self.assertFalse(qa.isEmpty)
        self.assertEqual(qa.satsInt, 100_000)
        ...
        qa = self.q.baseunitStrToAmount('1.0001')  # outside msat precision
        self.assertFalse(qa.isEmpty)
        self.assertEqual(qa.satsInt, 1)
        ...
        qa = self.q.baseunitStrToAmount('0.00000001001')
        self.assertFalse(qa.isEmpty)
        self.assertEqual(qa.satsInt, 1)
        self.assertEqual(qa.msatsInt, 1001)
