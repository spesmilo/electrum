...
class TestCommands(ElectrumTestCase):
    ...
    async def test_decrypt_enforces_strict_base64(self):
        cmds = Commands(config=self.config)
        wallet = restore_wallet_from_text__for_unittest(
            '9dk',
            path=None,
...
class TestCommandsTestnet(ElectrumTestCase):
    ...
    async def test_getprivatekeyforpath(self):
        wallet = restore_wallet_from_text__for_unittest(
    ...
    async def test_hold_invoice_commands(self):
    ...
    async def test_get_submarine_swap_providers(self):
        wallet = restore_wallet_from_text__for_unittest(
            'disagree rug lemon bean unaware square alone beach tennis exhibit fix mimic',
            path=None,
            config=self.config)['wallet']
    ...
    async def test_export_lightning_preimage(self):
