...
class PluginLoaderTestCase(ElectrumTestCase):
    ...
    def test_tampered_plugin_is_not_authorized(self):
        ...
        plugins = self._start_plugins()
        self._authorize(plugins)
        plugins._drop_zip_bundle(PLUGIN_NAME)  # as if electrum had been restarted
    ...
    def test_authorize_signs_the_hash_the_user_was_shown(self):
        make_plugin_zip(self.zip_path, secret='benign')
        plugins = self._start_plugins()
        # the file is swapped after the manifest was read, i.e. after the user
        # inspected the plugin but before they clicked authorize
        self._replace_zip_on_disk('evil')
        self._authorize(plugins)
        # what got signed is the benign hash, so the file on disk fails to
        # verify and the plugin does not load
        self.assertFalse(plugins.is_authorized(PLUGIN_NAME))
        self.assertIsNone(plugins.load_plugin_by_name(PLUGIN_NAME))

    def test_installing_another_archive_drops_the_cached_one(self):
        # the GUI's "already installed" check is on the filename, so a second
        # archive claiming the same plugin name can be installed alongside
        make_plugin_zip(self.zip_path, secret='v1')
        plugins = self._start_plugins()
        self._authorize(plugins)
        plugins.load_plugin_by_name(PLUGIN_NAME)

        other = os.path.join(self.plugins_dir, f'{PLUGIN_NAME}-2.zip')
        make_plugin_zip(other, secret='v2')
        plugins.add_external_plugin_metadata(plugins.read_manifest(other))

        # the metadata now describes the new archive, which was never signed,
        # so the plugin has to be authorized again rather than staying
        # authorized on the strength of the copy we still had in memory
        self.assertFalse(plugins.is_authorized(PLUGIN_NAME))

    def test_cancelling_an_install_drops_the_cached_archive(self):
        make_plugin_zip(self.zip_path, secret='v1')
        plugins = self._start_plugins()
        self._authorize(plugins)
        # the dialog shown during an install reads is_authorized(), which is
        # what builds and caches the archive
        self.assertTrue(plugins.is_authorized(PLUGIN_NAME))

        plugins.remove_external_plugin_metadata(PLUGIN_NAME)
...
class MemoryZipImporterTestCase(ElectrumTestCase):
    ...
    def test_submodule_search_location_is_not_a_filesystem_path(self):
        # if this finder were ever bypassed, PathFinder must not be able to
        # resolve the submodule from the zip on disk instead
