...
class TestWalletStorage(WalletTestCase):
    ...
    async def test_save_backup_overwrites_corrupt_backup(self):
        text = 'bitter grass shiver impose acquire brush forget axis eager alone wine silver'
        wallet = restore_wallet_from_text__for_unittest(text, path=self.wallet_path, config=self.config)['wallet']
        backup_path = os.path.join(self.electrum_path, wallet.basename() + '.backup')
        with open(backup_path, 'w') as f:
            f.write('{"seed_version": 5')  # a corrupt previous backup at the same path
        self.assertEqual(backup_path, wallet.save_backup(self.electrum_path))
        db = WalletDB(DictStorage(backup_path), upgrade=False)
        self.assertEqual(wallet.db.get('seed_version'), db.get('seed_version'))

    async def test_appends_after_incomplete_data_recovery_survive_reload(self):
        storage = DictStorage(self.wallet_path, allow_partial_writes=True)
        storage["a"] = "b"
        storage.write()
        ...
        storage = DictStorage(self.wallet_path, allow_partial_writes=True)
        self.assertEqual(storage.get("c"), "d")
