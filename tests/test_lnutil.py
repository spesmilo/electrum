...
from electrum.stored_dict import json_default, WalletStorage
...
class TestLNUtil(ElectrumTestCase):
    def test_shachain_store(self):
        ...
        for test in tests:
            ...
            receiver = RevocationStore(storage.get_stored_dict())
    ...
    def test_shachain_produce_consume(self):
        ...
        storage = WalletStorage(None)
        consumer = RevocationStore(storage.get_stored_dict())
        for i in range(10000):
            ...
            if i % 1000 == 0:
                c1 = consumer
                s1 = json.dumps(storage.json_data, default=json_default)
                storage2 = WalletStorage(None)
                storage2.set_data(s1)
                c2 = RevocationStore(storage2.get_stored_dict())
                s2 = json.dumps(storage2.json_data, default=json_default)
