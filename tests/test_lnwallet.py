...
from electrum.util import bfh, UserFacingException, OldTaskGroup
from electrum.storage import WalletStorage
from electrum.wallet import Abstract_Wallet
from electrum.wallet_db import WalletDB

from . import ElectrumTestCase, lnhelpers
from .lnhelpers import create_test_channels, find_free_port, SuccessfulTest, force_state_transition
...
class TestLNWallet(ElectrumTestCase):
    ...
    async def test_channel_open_rate_limit(self):
        ...
        async def run_test():
            ...
            # closing an unfunded channel must not free up budget
            await bob.close_channel(chan1.channel_id)
            ...
            self.assertTrue(any("rejecting incoming channel: per-peer rate limit" in msg for msg in logs.output))
            self.assertTrue(any("remote peer sent warning [DO NOT TRUST THIS MESSAGE]: 'rate limited, try again later'" in msg for msg in logs.output))
            ...
            self.assertTrue(any("rejecting incoming channel: global rate limit" in msg for msg in logs.output))
            self.assertTrue(any("remote peer sent warning [DO NOT TRUST THIS MESSAGE]: 'rate limited, try again later'" in msg for msg in logs.output))
