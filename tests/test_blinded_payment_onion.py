...
class TestPaymentRouteBlinding(ElectrumTestCase):
    ...
    def test_blinded_payment_onion(self):
        ...
        print('pubkeys:')
        payment_path_pubkeys = [x.node_id for x in route] + hops_pubkeys
        for i, pk in enumerate(payment_path_pubkeys):
            print(pk.hex())
        ...
        for i, h in enumerate(hops_data):
            print('---')
            print(h)
            mine = h.to_bytes().hex()[0:-64]
            ref = generate['full_route']['hops'][i]['payload']
        ...
        print(ONION_MESSAGE_PACKET.hex())
