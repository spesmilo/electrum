...
from electrum_ltc.invoices import (PR_UNPAID, PR_EXPIRED, PR_UNKNOWN, PR_PAID, PR_INFLIGHT,
...
from electrum_ltc.util import (parse_URI, InvalidBitcoinURI, InvoiceError,
                               maybe_extract_lightning_payment_identifier)
from electrum_ltc.lnurl import decode_lnurl, request_lnurl, callback_lnurl
...
class QEUserEnteredPayment(QEInvoice):
    ...
    def validate(self):
        ...
        if not bitcoin.is_address(self._recipient):
            self.validationError.emit('recipient', _('Invalid Litecoin address'))
            return
