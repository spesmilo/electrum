...
from electrum_ltc import storage, bitcoin, keystore, bip32, slip39, wallet
...
from electrum_ltc.util import (
    ...
    UserFacingException)
from electrum_ltc.transaction import (TxOutput, Transaction, PartialTransaction, PartialTxOutput,
                                      PartialTxInput, tx_from_any, TxOutpoint)
from electrum_ltc.mnemonic import seed_type
...
class TestWalletKeystoreAddressIntegrityForMainnet(ElectrumTestCase):
    ...
    def test_slip39_groups_128bit_bip84_native_segwit(self, mock_save_db):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'ltc1qaggygkqgqjjpt58zrmhvjz5m9dj8mjsh2n99sv')
        self.assertEqual(w.get_change_addresses()[0], 'ltc1q8l6hcvlczu4mtjcnlwhczw7vdxnvwccpkrtukj')
...
class TestWalletSending(TestCaseForTestnet):
    ...
    def test_sending_between_p2wpkh_and_compressed_p2pkh(self, mock_save_db):
        ...
        wallet1.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)  # TX_HEIGHT_UNCONF_PARENT but nvm
        wallet2.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
    ...
    def test_sending_between_p2sh_2of3_and_uncompressed_p2pkh(self, mock_save_db):
        ...
        wallet1a.adb.receive_tx_callback(funding_txid, funding_tx, TX_HEIGHT_UNCONFIRMED)
        ...
        wallet1a.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
        wallet2.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
    ...
    def test_sending_between_p2wsh_2of3_and_p2wsh_p2sh_2of2(self, mock_save_db):
        ...
        wallet1a.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
        wallet2a.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
    ...
    def _bump_fee_p2wpkh_when_there_is_only_a_single_output_and_that_is_a_change_address(self, *, simulate_moving_txs, config):
        ...
        # create tx
        outputs = [PartialTxOutput.from_address_and_value('tltc1q7rl9cxr85962ztnsze089zs8ycv52hk4gpn946', '!')]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins=coins, outputs=outputs, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325499
    ...
    def _rbf_batching(self, *, simulate_moving_txs, config):
        ...
        outputs = [PartialTxOutput.from_address_and_value('tltc1qy6xmdj96v5dzt3j08hgc05yk3kltqsnmhapynd', 2_500_000)]
        coins = wallet.get_spendable_coins(domain=None)
    ...
    def test_coinjoin_between_two_p2wpkh_electrum_seeds(self, mock_save_db):
        ...
        outputs = [PartialTxOutput.from_address_and_value("tltc1qhye4wfp26kn0l7ynpn5a4hvt539xc3zfkmuymc", 10_000_000)]
        ...
        outputs = [PartialTxOutput.from_address_and_value("tltc1qufnj5k2rrsnpjq7fg6d2pq3q9um6skdya34syj", 10_000_000)]
    ...
    def test_standard_wallet_cannot_sign_multisig_input_even_if_cosigner(self, mock_save_db):
        ...
        wallet_2of2.adb.receive_tx_callback(funding_txid, funding_tx, TX_HEIGHT_UNCONFIRMED)

        # create tx
        outputs = [PartialTxOutput.from_address_and_value('tltc1qfrlx5pza9vmez6vpx7swt8yp0nmgz3qa86sgvq', 100_000)]
        ...
        self.assertEqual('tltc1qxq5crk6yadw66rdt8xr3xj5ctvmq4c3z0fl85yx0ar8l6ga6ehysfvlnuf', tx.inputs()[0].address)
        self.assertEqual('tltc1qfrlx5pza9vmez6vpx7swt8yp0nmgz3qa86sgvq',                     tx.outputs()[0].address)
        self.assertEqual('tltc1qadpg5z77egkpkde34mdcrsz3s3tgwk5ew4w3wlfqf4j3dk8kkvrswgd6vs', tx.outputs()[1].address)
    ...
    def test_wallet_history_chain_of_unsigned_transactions(self, mock_save_db):
        ...
        # create tx1
        outputs = [PartialTxOutput.from_address_and_value('tltc1qsfcddwf7yytl62e3catwv8hpl2hs9e363z67kk', 100000)]
        coins = wallet.get_spendable_coins(domain=None)
        ...
        outputs = [PartialTxOutput.from_address_and_value('tltc1qq0lm9esmq6pfjc3jls7v6twy93lnqcs8dxa48c', '!')]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins=coins, outputs=outputs, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1938863
    ...
    def test_wallet_does_not_create_zero_input_tx(self, mock_save_db):
        ...
        with self.subTest(msg="no coins to use as inputs, max output value, zero fee"):
            outputs = [PartialTxOutput.from_address_and_value('tltc1qsfcddwf7yytl62e3catwv8hpl2hs9e363z67kk', '!')]
            coins = wallet.get_spendable_coins(domain=None)
        ...
        with self.subTest(msg="funded wallet, zero output value, zero fee"):
            outputs = [PartialTxOutput.from_address_and_value('tltc1qsfcddwf7yytl62e3catwv8hpl2hs9e363z67kk', 0)]
            coins = wallet.get_spendable_coins(domain=None)
            tx = wallet.make_unsigned_transaction(coins=coins, outputs=outputs, fee=0)
            self.assertEqual(1, len(tx.inputs()))
    ...
    def test_imported_wallet_usechange_off(self, mock_save_db):
        ...
        outputs = [PartialTxOutput.from_address_and_value('tltc1qq4pypzwxf5uanfyckmsu3ejxxf6rrvjqplqrp9', 49646)]
        coins = wallet.get_spendable_coins(domain=None)
        ...
        self.assertEqual(2, len(tx.outputs()))
        self.assertTrue(tx.output_value_for_address("tltc1q0fj7pxa3m2q2hlr964zn3z3wvx4t03epdp2d8d") > 0)
        self.assertEqual(49646, tx.output_value_for_address("tltc1qq4pypzwxf5uanfyckmsu3ejxxf6rrvjqplqrp9"))
    ...
    def test_imported_wallet_usechange_on(self, mock_save_db):
        ...
        self.assertEqual(2, len(tx.outputs()))
        self.assertTrue(tx.output_value_for_address("tltc1qetcgdwuzlpdnt5fmzxxdpczjhadz06cy2ff439") > 0)
        self.assertEqual(49646, tx.output_value_for_address("tltc1qq4pypzwxf5uanfyckmsu3ejxxf6rrvjqplqrp9"))
    ...
    def test_imported_wallet_usechange_on__no_more_unused_addresses(self, mock_save_db):
        ...
        for txid, rawtx in _txs:
            ...
            self.assertEqual(txid, tx.txid())
            wallet.adb.receive_tx_callback(txid, tx, TX_HEIGHT_UNCONFIRMED)
    ...
    def test_get_spendable_coins(self, mock_save_db):
        ...
        self.assertEqual(
            {'52e669a20a26c8b3df5b41e5e6309b18bcde8e1ad7ea17a18f63b6dc6c8becc0:1'},
            {txi.prevout.to_str() for txi in wallet.get_spendable_coins(["tltc1q6n99dl96mx8mfh90m3tn5awk5mllkzdhnu0swt"])})
        # test freezing an address
        wallet.set_frozen_state_of_addresses(["tltc1q6n99dl96mx8mfh90m3tn5awk5mllkzdhnu0swt"], freeze=True)
        self.assertEqual(
            {'c36a6e1cd54df108e69574f70bc9b88dc13beddc70cfad9feb7f8f6593255d4a:1'},
            {txi.prevout.to_str() for txi in wallet.get_spendable_coins()})
        wallet.set_frozen_state_of_addresses(["tltc1q6n99dl96mx8mfh90m3tn5awk5mllkzdhnu0swt"], freeze=False)
        self.assertEqual(
            {'c36a6e1cd54df108e69574f70bc9b88dc13beddc70cfad9feb7f8f6593255d4a:1',
             '52e669a20a26c8b3df5b41e5e6309b18bcde8e1ad7ea17a18f63b6dc6c8becc0:1'},
            {txi.prevout.to_str() for txi in wallet.get_spendable_coins()})
    ...
    def test_export_psbt_with_xpubs__multisig(self, mock_save_db):
        ...
        outputs = [PartialTxOutput.from_address_and_value("tltc1q0ezagv55krljkz9973fryeyczhj3dnls3td5ch", 123456)]
        coins = wallet.get_spendable_coins(domain=None)
        ...
        self.assertEqual(
            {'022c4338968f87a09b0fefd0aaac36f1b983bab237565d521944c60fdc48275049': ('9559fbd1', "m/9999'/0/0"),
        ...
        self.assertEqual(
            {'tpubD6NzVbkrYhZ4WW1saJM1hDjGz1rm5swdKwbhcsx9hW5VVXDdbnt6GbXEQVXQq97dYsvGVeMEw5Ge2Zx4QGBy6W5KXahih4aTRs5hLqgy9c9': ('015148ee', 'm'),
             'tpubD9MoDeHnEQnU5EMgt9mc4yKU6SURbfq2ooMToY5GH95B8Li1CEsuo9dBKXM2sdjuDGq4KCXLuigss3y22fZULzVrfVuZDxEN55Sp6CcU9DK': ('9559fbd1', "m/9999'")},
            tx.to_json()['xpubs'])
        ...
        self.assertEqual(
            {'022c4338968f87a09b0fefd0aaac36f1b983bab237565d521944c60fdc48275049': ('9559fbd1', "m/9999'/0/0"),
             '03cf9a6ac058d36a6dc325b19715a2223c6416e1cef13bc047a99bded8c99463ca': ('30cf1be5', "m/48'/1'/0'/2'/0/0")},
            tx.inputs()[0].to_json()['bip32_paths'])
        ...
        tx.prepare_for_export_for_hardware_device(wallet)
        self.assertEqual(
            {'tpubDFF7YPCSGHZy55HkQj6HJkXCR8DWbKKXpTYBH38fSHf6VuoEzNmZQZdAoKEVy36S8zXkbGeV4XQU6vaRXGsQfgptFYPR4HSpAenqkY7J7Lg': ('30cf1be5', "m/48'/1'/0'/2'"),
             'tpubD9MoDeHnEQnU5EMgt9mc4yKU6SURbfq2ooMToY5GH95B8Li1CEsuo9dBKXM2sdjuDGq4KCXLuigss3y22fZULzVrfVuZDxEN55Sp6CcU9DK': ('9559fbd1', "m/9999'")},
            tx.to_json()['xpubs'])
    ...
    def test_export_psbt_with_xpubs__singlesig(self, mock_save_db):
        ...
        self.assertEqual({}, tx.to_json()['xpubs'])
        self.assertEqual(
            {'029e65093d22877cbfcc27cb754c58d144ec96635af1fcc63e5a7b90b23bb6acb8': ('30cf1be5', "m/84'/1'/0'/0/0")},
            tx.inputs()[0].to_json()['bip32_paths'])
...
class TestWalletOfflineSigning(TestCaseForTestnet):
    ...
    def test_sending_offline_old_electrum_seed_online_mpk(self, mock_save_db):
        ...
        # create unsigned tx
        outputs = [PartialTxOutput.from_address_and_value('tltc1qyw3c0rvn6kk2c688y3dygvckn57525y8emy4p5', 2500000)]
    ...
    def test_sending_offline_xprv_online_xpub_p2pkh(self, mock_save_db):
        ...
        # create unsigned tx
        outputs = [PartialTxOutput.from_address_and_value('tltc1qp0mv2sxsyxxfj5gl0332f9uyez93su9cszcqyh', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325340
        tx.version = 1
    ...
    def test_sending_offline_wif_online_addr_p2pkh(self, mock_save_db):  # compressed pubkey
        ...
        # create unsigned tx
        outputs = [PartialTxOutput.from_address_and_value('tltc1quk7ahlhr3qmjndy0uvu9y9hxfesrtahtj48k8j', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325340
        tx.version = 1
...
class TestWalletHistory_SimpleRandomOrder(TestCaseForTestnet):
    ...
    def test_restoring_old_wallet_txorder1(self, mock_save_db):
        ...
        for i in [2, 12, 7, 9, 11, 10, 16, 6, 17, 1, 13, 15, 5, 8, 4, 0, 14, 18, 3]:
            ...
            w.adb.receive_tx_callback(tx.txid(), tx, TX_HEIGHT_UNCONFIRMED)
...
class TestWalletHistory_EvilGapLimit(TestCaseForTestnet):
    ...
    def test_restoring_wallet_txorder1(self, mock_save_db):
        ...
        w.adb.receive_history_callback('tltc1qgh5c088he4d559wl0hw27hrdeg8p2z96c3td9f',  # HD index 1
                                   [('268fce617aaaa4847835c2212b984d7b7741fdab65de22813288341819bc5656', 1316917)],
                                   {})
        w.synchronize()
        w.adb.receive_history_callback('tltc1qm0ejr6g964zt2jux5te7m9ds43n28hds528z0k',  # HD index 3
                                   [('511a35e240f4c8855de4c548dad932d03611a37e94e9203fdb6fc79911fe1dd4', 1316912)],
                                   {})
        w.synchronize()
        w.adb.receive_history_callback('tltc1qj4pnq958k89zcem3342lhcgyz0rnmhkzxjy3gk',  # HD index 4
                                   [('fde0b68938709c4979827caa576e9455ded148537fdb798fd05680da64dc1b4f', 1316917)],
                                   {})
        w.synchronize()
        w.adb.receive_history_callback('tltc1q3pyjwpm8wxgvquak240mprfhaydmkawcfhg2zs',  # HD index 15
                                   [('511a35e240f4c8855de4c548dad932d03611a37e94e9203fdb6fc79911fe1dd4', 1316912)],
                                   {})
        w.synchronize()
        w.adb.receive_history_callback('tltc1qr0qjp99ygawul0eylxfqmt7alygye22mtenjfm',  # HD index 25
                                   [('fde0b68938709c4979827caa576e9455ded148537fdb798fd05680da64dc1b4f', 1316917),
                                    ('268fce617aaaa4847835c2212b984d7b7741fdab65de22813288341819bc5656', 1316917)],
                                   {})
        w.synchronize()
...
class TestImportedWallet(TestCaseForTestnet):
    ...
    def test_importing_and_deleting_addresses(self, mock_save_db):
        w = restore_wallet_from_text("tltc1q7648a2pm2se425lvun0g3vlf4ahmflctw32u2c",
                                     path='if_this_exists_mocking_failed_648151893',
                                     config=self.config)['wallet']  # type: Abstract_Wallet
        ...
        w.import_address("tltc1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad")
        ...
        w.delete_address("tltc1q7648a2pm2se425lvun0g3vlf4ahmflctw32u2c")
        self.assertEqual(2, len(w.db.transactions))
        self.assertEqual(
        ...
        with self.assertRaises(UserFacingException) as ctx:
            w.delete_address("tltc1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad")
